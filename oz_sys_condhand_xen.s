...
##########################################################################
	...
	.include "oz_params_xen.s"	# to get MCH_... symbols and PBL_EVENTS/POP_EVENTS macros
...
oz_sys_condhand_call:
	...
	pushl	12(%ebp)		# if we are attached by a debugger, 
	pushl	 8(%ebp)		# ... let it know about the exception
	call	oz_sys_thread_halted
	addl	$8,%esp
	cmpl	oz_RESUME,%eax		# see if it wants us to retry the instruction
	je	call_resume

	movw	%cs,%dx			# if not, search for an exception handler
	movl	(%ebp),%ecx
call_loop:
	movl	%ecx,%ebx
	movl	(%ecx),%ecx
