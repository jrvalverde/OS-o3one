...
typedef struct Attachedthread Attachedthread;
...
struct Attachedthread { Attachedthread *next;
                        OZ_Threadid thread_id;
                        OZ_Handle h_thread;
                        OZ_Handle h_waitevent;
                        OZ_Handle h_haltevent;
                        OZ_Procmode procmode;
                        OZ_Sigargs const *volatile sigargs_p;
                        OZ_Mchargs *volatile mchargs_p;
                        OZ_Mchargx *volatile mchargx_p;
                        OZ_Processid volatile processid;
                      };
...
static Attachedthread *attachedthreads;
...
static uLong int_halt_thread          (OZ_Handle h_input, OZ_Handle h_output, OZ_Handle h_error, char *name, void *dummy, int argc, const char *argv[]);
...
static uLong int_step_thread          (OZ_Handle h_input, OZ_Handle h_output, OZ_Handle h_error, char *name, void *dummy, int argc, const char *argv[]);
...
static Command intcmd[] = {
	...
	1, "halt thread",          int_halt_thread,          NULL, "[-id <thread_id>] [<thread_logical_name>]", 
	...
	1, "show volume",          int_show_volume,          NULL, "[<device_name> ...]", 
        1, "step thread",          int_step_thread,          NULL, "[-id <thread_id>] [<thread_logical_name>]", 
...

{
  ...
  int i;
  uLong cmdlen, sts;
  OZ_Handle h_event, h_logname, h_logtable;
  Symbol *symbol;
  ...
  for (i = 1; i < argc; i ++) {
    ...
    if (strcmp (argv[i], "-console") == 0) {
      ...
      sts = oz_sys_logname_lookup (0, OZ_PROCMODE_USR, defaulttables, NULL, NULL, NULL, &h_logtable);
      if (sts == OZ_SUCCESS) {
        sts = oz_sys_logname_lookup (h_logtable, OZ_PROCMODE_USR, argv[i], NULL, NULL, NULL, &h_logname);
        oz_sys_handle_release (OZ_PROCMODE_KNL, h_logtable);
        if (sts == OZ_SUCCESS) {
          sts = oz_sys_logname_getval (h_logname, 0, NULL, 0, NULL, NULL, &h_s_console, OZ_OBJTYPE_IOCHAN, NULL);
...

{
  ...
  if (tc -> command != NULL) {
    ...
    if (tc -> command -> async) {
      if ((tc -> runopts.debug) || (tc -> runopts.defdir != NULL) || (tc -> runopts.job_name != NULL) || (tc -> runopts.orphan) || (tc -> runopts.process_name != NULL)) {
        oz_sys_io_fs_printf (h_s_error, "oz_cli: -debug, -directory, -job, -orphan and -process options illegal on '%s' command\n", tc -> command -> name);
    ...
    /* Otherwise, they are simply called */
      ...
      if ((tc -> runopts.debug) || (tc -> runopts.defdir != NULL) || (tc -> runopts.job_name != NULL) || (tc -> runopts.process_name != NULL) || (tc -> runopts.timeit) || (tc -> runopts.orphan) 
       || (tc -> runopts.exit_name != NULL) || (tc -> runopts.init_name != NULL) || (tc -> runopts.thread_name != NULL) || !(tc -> runopts.wait)) {
        oz_sys_io_fs_printf (h_s_error, "oz_cli: -debug -directory, -exit, -init, -job, -nowait, -orphan, -process, -thread and -timeit options illegal on '%s' command\n", tc -> command -> name);
        cleanup_runopts (&(tc -> runopts));
        free (tc);
        return (OZ_BADPARAM);
      }
...
/************************************************************************/
/*									*/
/*	halt thread [<logical_name> / -id <thread_id>]			*/
/*									*/
/************************************************************************/

static uLong wait_attachedthread_halt (OZ_Handle h_output, OZ_Handle h_error, Attachedthread *attachedthread);
static uLong print_mchargs (void *h_outputv, char const *format, ...);

static uLong int_halt_thread (OZ_Handle h_input, OZ_Handle h_output, OZ_Handle h_error, char *name, void *dummy, int argc, const char *argv[])

{
  Attachedthread *attachedthread;
  char event_name[OZ_EVENT_NAMESIZE];
  char const *logical_name;
  int i, usedup;
  OZ_Handle h_thread;
  OZ_Threadid my_thread_id, thread_id;
  uLong sts;
  ...
  /* Get handle to thread and its id from logical name or id number */
  ...
  if (thread_id != 0) {
    ...
    sts = logname_getobj (logical_name, OZ_OBJTYPE_THREAD, &h_thread);
    if (sts != OZ_SUCCESS) return (sts);
    sts = oz_sys_thread_getid (h_thread, &thread_id);
    if (sts != OZ_SUCCESS) oz_sys_condhand_signal (2, sts, 0);
  }

  /* Can't halt self (duh) */

  sts = oz_sys_thread_getid (0, &my_thread_id);
  if (sts != OZ_SUCCESS) oz_sys_condhand_signal (2, sts, 0);
  if (thread_id == my_thread_id) {
    oz_sys_io_fs_printf (h_error, "oz_cli: cannot halt self\n", sts);
    oz_sys_handle_release (OZ_PROCMODE_KNL, h_thread);
    return (OZ_BADPARAM);
  }

  /* See if we already have the thread attached.  If not, attach it. */

  for (attachedthread = attachedthreads; attachedthread != NULL; attachedthread = attachedthread -> next) {
    if (attachedthread -> thread_id == thread_id) break;
  }
  if (attachedthread == NULL) {
    attachedthread = malloc (sizeof *attachedthread);
    attachedthread -> next = attachedthreads;
    attachedthread -> thread_id = thread_id;
    attachedthread -> h_thread  = h_thread;
    oz_sys_sprintf (sizeof event_name, event_name, "halted by %u", my_thread_id);
    sts = oz_sys_event_create (OZ_PROCMODE_KNL, event_name, &(attachedthread -> h_waitevent));
    if (sts != OZ_SUCCESS) oz_sys_condhand_signal (2, sts, 0);
    oz_sys_sprintf (sizeof event_name, event_name, "halt of %u", thread_id);
    sts = oz_sys_event_create (OZ_PROCMODE_KNL, event_name, &(attachedthread -> h_haltevent));
    if (sts != OZ_SUCCESS) oz_sys_condhand_signal (2, sts, 0);
    attachedthread -> procmode  = OZ_PROCMODE_USR;
    attachedthread -> sigargs_p = NULL;
    attachedthread -> mchargs_p = NULL;
    attachedthread -> mchargx_p = NULL;
    attachedthread -> processid = -1;

    oz_sys_handle_setthread (OZ_PROCMODE_KNL, attachedthread -> h_thread,    0);
    oz_sys_handle_setthread (OZ_PROCMODE_KNL, attachedthread -> h_waitevent, 0);
    oz_sys_handle_setthread (OZ_PROCMODE_KNL, attachedthread -> h_haltevent, 0);

    sts = oz_sys_thread_attach (h_thread, OZ_PROCMODE_USR, 
                                          attachedthread -> h_waitevent, 
                                          attachedthread -> h_haltevent, 
                                          &(attachedthread -> sigargs_p), 
                                          &(attachedthread -> mchargs_p), 
                                          &(attachedthread -> mchargx_p), 
                                          &(attachedthread -> processid));
    if (sts != OZ_SUCCESS) {
      oz_sys_io_fs_printf (h_error, "oz_cli: error %u attaching thread\n", sts);
      free (attachedthread);
      return (sts);
    }
    attachedthreads = attachedthread;
  }

  /* If already attached, we don't need new handle, just use attachedthread -> h_thread */

  else oz_sys_handle_release (OZ_PROCMODE_KNL, h_thread);

  /* Tell the thread to halt at the attached access mode */

  sts = oz_sys_thread_halt (attachedthread -> h_thread, attachedthread -> procmode);
  if (sts != OZ_SUCCESS) {
    oz_sys_io_fs_printf (h_error, "oz_cli: error %u halting thread\n", sts);
    return (sts);
  }

  /* Wait for thread to halt - it writes sigargs_p,mchargs_p,processid then sets h_haltevent */

  sts = wait_attachedthread_halt (h_output, h_error, attachedthread);
  return (sts);
}

/************************************************************************/
...

{
  ...
  oz_sys_io_fs_printf (h_output, "\n  all can be prefixed by any of the following options:\n");
  oz_sys_io_fs_printf (h_output, "    -debug\n");
...

{
  ...
  flags = 0;
  if (runopts -> debug) flags = OZ_SYS_SPAWN_FLAG_DEBUG;

  sts = oz_sys_spawn2 (runopts -> h_job, 
                       image, 
                       runopts -> h_in, 
                       runopts -> h_out, 
                       runopts -> h_err, 
...

{
  ...
  for (i = 0; i < argc; i ++) {
    ...
    /* Specify to start debugger - this tells oz_sys_spawn to create process logical OZ_DEBUG 1    */
    /* When the program starts, oz_crtl_start or oz_util_start see this and call oz_usr_debug_init */
...

{
  ...
  OZ_Handle_item items[] = { 
	OZ_HANDLE_CODE_SYSTEM_KNLVERINT,     sizeof s_knlverint,     &s_knlverint,     NULL, 
  ...
  if (sts != OZ_SUCCESS) oz_sys_io_fs_printf (h_error, "oz_cli: error %u getting system info [%u]\n", sts, index);
    ...
    oz_sys_io_fs_printf (h_output, "                 kernel version : %X\n", s_knlverint);
...
/************************************************************************/
/*									*/
/*	step thread [<logical_name> / -id <thread_id>]			*/
/*		[-set <name>=<value>]					*/
/*									*/
/*	Steps a previously halted thread				*/
/*									*/
/************************************************************************/
...
{
  Attachedthread *attachedthread;
  char const *logical_name, *p;
  int i, j, l, nsets, usedup;
  OZ_Handle h_thread, h_process;
  OZ_Pointer value;
  OZ_Threadid thread_id;
  struct { int index;
           OZ_Pointer value;
  ...
  static uQuad const onebits = -1LL;
  ...
  nsets = 0;
  sets  = malloc (argc / 2 * sizeof *sets);
  ...
  for (i = 0; i < argc; i ++) {
    ...
    if (strcasecmp (argv[i], "-set") == 0) {
      ...
      /* Make sure a register=value is next */

      if ((++ i >= argc) || (*argv[i] == '-')) {
        oz_sys_io_fs_printf (h_error, "oz_cli: missing name=value after -set\n");
        sts = OZ_MISSINGPARAM;
        goto rtnsts;
      }
      p = strchr (argv[i], '=');
      if (p == NULL) {
        oz_sys_io_fs_printf (h_error, "oz_cli: no = in name=value in -set %s\n", argv[i]);
        sts = OZ_BADPARAM;
        goto rtnsts;
      }
      l = p - argv[i];

      /* Check for register name in standard mchargs */

      for (j = 0; oz_hw_mchargs_des[j].name != NULL; j ++) {
        if ((oz_hw_mchargs_des[j].name[l] == 0) && (strncasecmp (oz_hw_mchargs_des[j].name, argv[i], l) == 0)) break;
      }
      if (oz_hw_mchargs_des[j].name != NULL) {
        value = oz_hw_atoz (++ p, &usedup);
        if (p[usedup] != 0) {
          oz_sys_io_fs_printf (h_error, "oz_cli: bad value in %s\n", argv[i]);
          sts = OZ_BADPARAM;
          goto rtnsts;
        }
        sets[nsets].index = j;
        sets[nsets].value = value;
        sets[nsets].arg   = argv[i];
        nsets ++;
        continue;
      }

      /* Check for register name in extended mchargs */

      for (j = 0; oz_hw_mchargx_des[j].name != NULL; j ++) {
        if ((oz_hw_mchargx_des[j].name[l] == 0) && (strncasecmp (oz_hw_mchargx_des[j].name, argv[i], l) == 0)) break;
      }
      if (oz_hw_mchargx_des[j].name != NULL) {
        value = oz_hw_atoz (++ p, &usedup);
        if (p[usedup] != 0) {
          oz_sys_io_fs_printf (h_error, "oz_cli: bad value in %s\n", argv[i]);
          sts = OZ_BADPARAM;
      ...
      oz_sys_io_fs_printf (h_error, "oz_cli: bad register name in %s\n", argv[i]);
      sts = OZ_BADPARAM;
      goto rtnsts;
    }
  ...
  /* If logical name, get corresponding thread-id */
  ...
  /* We must have the thread attached */

  for (attachedthread = attachedthreads; attachedthread != NULL; attachedthread = attachedthread -> next) {
    if (attachedthread -> thread_id == thread_id) break;
  }
  if (attachedthread == NULL) {
    oz_sys_io_fs_printf (h_error, "oz_cli: thread %u not attached\n", thread_id);
    sts = OZ_BADPARAM;
    goto rtnsts;
  }
  ...
  if (nsets > 0) {
    sts = oz_sys_process_getbyid (attachedthread -> processid, &h_process);
    if (sts != OZ_SUCCESS) {
      oz_sys_io_fs_printf (h_error, "oz_cli: error %u getting process-id %u handle\n", sts, attachedthread -> processid);
      goto rtnsts;
    }
    ...
    for (i = 0; i < nsets; i ++) {
      ...
      if (j < 0) {
        j = ~j;
        sts = oz_sys_process_poke (h_process, OZ_PROCMODE_KNL, oz_hw_mchargx_des[j].size, &(sets[i].value), 
                                   ((uByte *)(attachedthread -> mchargx_p + 0)) + oz_hw_mchargx_des[j].offs);
        if (sts == OZ_SUCCESS) {
          sts = oz_sys_process_poke (h_process, OZ_PROCMODE_KNL, oz_hw_mchargx_des[j].size, &onebits, 
                                     ((uByte *)(attachedthread -> mchargx_p + 1)) + oz_hw_mchargx_des[j].offs);
        }
      } else {
        sts = oz_sys_process_poke (h_process, OZ_PROCMODE_KNL, oz_hw_mchargs_des[j].size, &(sets[i].value), 
                                   ((uByte *)(attachedthread -> mchargs_p)) + oz_hw_mchargs_des[j].offs);
      }
      if (sts != OZ_SUCCESS) {
        oz_sys_io_fs_printf (h_error, "oz_cli: error %u writing %s to process\n", sets[i].arg, sts);
        goto rtnsts;
      }
    }
  }

  /* When it halts again, make sure it fills all this stuff in and we see it set the event flag */

  attachedthread -> sigargs_p = NULL;
  attachedthread -> mchargs_p = NULL;
  attachedthread -> processid = 0;
  oz_sys_event_set (OZ_PROCMODE_KNL, attachedthread -> h_haltevent, 0, NULL);

  /* Resume the thread by setting the event flag.  Tell it to resume at point of interruption and that we want to stay attached. */

  oz_sys_event_set (OZ_PROCMODE_KNL, attachedthread -> h_waitevent, (OZ_THREAD_HALT_RESUME | OZ_THREAD_HALT_ATTACH) + 1, NULL);

  /* Wait for thread to halt again and print info */

  sts = wait_attachedthread_halt (h_output, h_error, attachedthread);

rtnsts:
  if (h_process != 0) oz_sys_handle_release (OZ_PROCMODE_KNL, h_process);
  free (sets);
  return (sts);
}
...
{
  Attachedthread **lattachedthread, *xattachedthread;
  int i;
  Long haltvalue;
  OZ_Handle h_process;
  OZ_Mchargs mchargs;
  OZ_Mchargx mchargx;
  OZ_Sigargs sigarg0, sigargn;
  uLong sts;

  /* Wait for thread to halt - it writes sigargs_p,mchargs_p,processid then sets h_haltevent */

  while ((sts = oz_sys_event_inc (OZ_PROCMODE_KNL, attachedthread -> h_haltevent, 0, &haltvalue)) == OZ_SUCCESS) {
    if (haltvalue > 0) break;
    oz_sys_event_wait (OZ_PROCMODE_KNL, attachedthread -> h_haltevent, 0);
  }
  if (sts != OZ_SUCCESS) oz_sys_condhand_signal (2, sts, 0);

  /* Print results of halt */

  h_process = 0;
  if (attachedthread -> sigargs_p == NULL) oz_sys_io_fs_printf (h_error, "oz_cli: thread exited\n");
  else {
    sts = oz_sys_process_getbyid (attachedthread -> processid, &h_process);
    if (sts != OZ_SUCCESS) {
      oz_sys_io_fs_printf (h_error, "oz_cli: error %u getting process-id %u handle\n", sts, attachedthread -> processid);
    ...
    oz_sys_io_fs_printf (h_output, "oz_cli: sigargs %p:", attachedthread -> sigargs_p);
    sigarg0 = 0;
    for (i = 0; (i <= sigarg0) && (i < 100); i ++) {
      sts = oz_sys_process_peek (h_process, OZ_PROCMODE_KNL, sizeof sigargn, attachedthread -> sigargs_p + i, &sigargn);
      if (sts != OZ_SUCCESS) {
        oz_sys_io_fs_printf (h_output, " (exception %u)", sts);
        break;
      }
      if (i == 0) sigarg0 = sigargn;
      oz_sys_io_fs_printf (h_output, " %u", sigargn);
    }
    oz_sys_io_fs_printf (h_output, "\n");
  }

  if (attachedthread -> mchargs_p == NULL) oz_sys_io_fs_printf (h_error, "oz_cli: thread returned no mchargs\n");
  else {
    if (h_process == 0) {
      sts = oz_sys_process_getbyid (attachedthread -> processid, &h_process);
      if (sts != OZ_SUCCESS) {
    ...
    sts = oz_sys_process_peek (h_process, OZ_PROCMODE_KNL, sizeof mchargs, attachedthread -> mchargs_p, &mchargs);
    if (sts != OZ_SUCCESS) oz_sys_io_fs_printf (h_error, "oz_cli: mchargs %p: (exception %u)\n", mchargs, sts);
    else {
      oz_sys_io_fs_printf (h_output, "oz_cli: mchargs %p:\n", attachedthread -> mchargs_p);
      oz_hw_mchargs_print (print_mchargs, &h_output, 1, &mchargs);
      if (attachedthread -> mchargx_p == NULL) oz_sys_io_fs_printf (h_error, "oz_cli: thread returned no mchargx\n");
      else {
        sts = oz_sys_process_peek (h_process, OZ_PROCMODE_KNL, sizeof mchargx, attachedthread -> mchargx_p, &mchargx);
        if (sts != OZ_SUCCESS) oz_sys_io_fs_printf (h_error, "oz_cli: mchargx %p: (exception %u)\n", mchargx, sts);
        else {
          oz_sys_io_fs_printf (h_output, "oz_cli: mchargx %p:\n", attachedthread -> mchargx_p);
          oz_hw_mchargx_print (print_mchargs, &h_output, 1, &mchargs, &mchargx);
        }
      }
    }
  }
  ...
  /* If thread has exited, we are no longer attached */

  if (attachedthread -> sigargs_p == NULL) {
    for (lattachedthread = &attachedthreads; (xattachedthread = *lattachedthread) != NULL; lattachedthread = &(attachedthread -> next)) {
      if (xattachedthread == attachedthread) {
        *lattachedthread = xattachedthread -> next;
        oz_sys_handle_release (OZ_PROCMODE_KNL, xattachedthread -> h_thread);
        oz_sys_handle_release (OZ_PROCMODE_KNL, xattachedthread -> h_waitevent);
        oz_sys_handle_release (OZ_PROCMODE_KNL, xattachedthread -> h_haltevent);
        free (xattachedthread);
        break;
      }
    }
  }
...

{
  ...
  va_start (ap, format);
  sts = oz_sys_io_fs_vprintf (*(OZ_Handle *)h_outputv, format, ap);
  va_end (ap);
  return (sts);
}

