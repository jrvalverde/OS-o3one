...
struct Devex { int record;			/* 0 : stream-oriented; 1 : record-orented */
               ...
               int didsomething;		/* temp flag used to indicate some data was passed somewhere */
...
static void checkqueues (Devex *devex);
...

{
  ...
  switch (funcode) {
    ...
    case OZ_IO_FS_READREC: {
      ...
      checkqueues (devex);						/* there may be room in pipe buffer for more data, so check pending writes */
...
/************************************************************************/
/*									*/
/*  Process any requests lingering in the queues			*/
/*									*/
/************************************************************************/
...

{
  ...
  do {
    devex -> didsomething = 0;

    /* Try to process a write request */
    ...
    if (iopex != NULL) {
      ...
      /* If it completed, remove from queue and post completion, then try to complete another request */
    ...
    /* See if there are any reads to process */
...

{
  ...
  while (size > 0) {						/* repeat while there is stuff to do */
    ...
    devex -> didsomething = 1;					/* we moved some data */
  }
...

{
  ...
  if (iopex -> trmsize == 0) {
    while (devex -> remove != devex -> insert) {				/* repeat while there is data in pipe buffer */
      ...
      if (dlen == 255) goto eofmark;						/* 255 is an eof mark */
...
multibytenoterm:								/* terminator did not match pipe buffer contents */
      ...
      devex -> didsomething = 1;						/* we moved a byte of data */
    }
  }
  return (OZ_PENDING);								/* request not yet complete */
...
eofmark:
  if (iopex -> done == 0) {							/* if we hit it first off, return eof status */
    devex -> didsomething = 1;							/* we moved some data */
    return (OZ_ENDOFFILE);
  }
  if (devex -> remove == 0) devex -> remove = OZ_IO_PIPE_BUFSIZ;		/* we have data, put eof mark back in pipe buffer */
  -- (devex -> remove);
  return ((iopex -> trmsize == 0) ? OZ_SUCCESS : OZ_NOTERMINATOR);		/* no terminator for the data we did get, though */
}

/************************************************************************/
/*									*/
