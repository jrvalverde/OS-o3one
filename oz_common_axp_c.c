...
/* Transfer from cpu to mchargx struct */

void oz_hw_mchargx_fetch (OZ_Mchargx *mchargx)

{
  memset (mchargx, 0, sizeof *mchargx);
  if ((OZ_HWAXP_RD_PS () & 0x18) == 0) {
    mchargx -> asn_ro   = OZ_HWAXP_MFPR_ASN   ();
    mchargx -> asten_rw = OZ_HWAXP_MFPR_ASTEN ();
    mchargx -> astsr_rw = OZ_HWAXP_MFPR_ASTSR ();
    mchargx -> fen_rw   = OZ_HWAXP_MFPR_FEN   ();
    mchargx -> mces_rw  = OZ_HWAXP_MFPR_MCES  ();
    mchargx -> pcbb_ro  = OZ_HWAXP_MFPR_PCBB  ();
    mchargx -> prbr_rw  = OZ_HWAXP_MFPR_PRBR  ();
    mchargx -> ptbr_ro  = OZ_HWAXP_MFPR_PTBR  ();
    mchargx -> scbb_rw  = OZ_HWAXP_MFPR_SCBB  ();
    mchargx -> sisr_ro  = OZ_HWAXP_MFPR_SISR  ();
    mchargx -> usp_rw   = OZ_HWAXP_MFPR_USP   ();
    mchargx -> vptb_rw  = OZ_HWAXP_MFPR_VPTB  ();
    mchargx -> whami_ro = OZ_HWAXP_MFPR_WHAMI ();
  }
}

/* Transfer from mchargx struct to cpu */

void oz_hw_mchargx_store (OZ_Mchargx *mchargx, OZ_Mchargx *mchargx_mask)

{
  if ((OZ_HWAXP_RD_PS () & 0x18) == 0) {
    if (mchargx_mask -> asten_rw) OZ_HWAXP_MTPR_ASTEN (mchargx -> asten_rw);
    if (mchargx_mask -> astsr_rw) OZ_HWAXP_MTPR_ASTSR (mchargx -> astsr_rw);
    if (mchargx_mask -> datfx_wo) OZ_HWAXP_MTPR_DATFX (mchargx -> datfx_wo);
    if (mchargx_mask -> fen_rw)   OZ_HWAXP_MTPR_FEN   (mchargx -> fen_rw);
    if (mchargx_mask -> ipir_wo)  OZ_HWAXP_MTPR_IPIR  (mchargx -> ipir_wo);
    if (mchargx_mask -> mces_rw)  OZ_HWAXP_MTPR_MCES  (mchargx -> mces_rw);
    if (mchargx_mask -> prbr_rw)  OZ_HWAXP_MTPR_PRBR  (mchargx -> prbr_rw);
    if (mchargx_mask -> scbb_rw)  OZ_HWAXP_MTPR_SCBB  (mchargx -> scbb_rw);
    if (mchargx_mask -> sirr_wo)  OZ_HWAXP_MTPR_SIRR  (mchargx -> sirr_wo);
    if (mchargx_mask -> tbia_wo)  OZ_HWAXP_MTPR_TBIA  ();
    if (mchargx_mask -> tbis_wo)  OZ_HWAXP_MTPR_TBIS  (mchargx -> tbis_wo);
    if (mchargx_mask -> tbisd_wo) OZ_HWAXP_MTPR_TBISD (mchargx -> tbisd_wo);
    if (mchargx_mask -> tbisi_wo) OZ_HWAXP_MTPR_TBISI (mchargx -> tbisi_wo);
    if (mchargx_mask -> usp_rw)   OZ_HWAXP_MTPR_USP   (mchargx -> usp_rw);
    if (mchargx_mask -> vptb_rw)  OZ_HWAXP_MTPR_VPTB  (mchargx -> vptb_rw);
  }
}

/************************************************************************/
