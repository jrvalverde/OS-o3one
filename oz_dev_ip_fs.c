...

{
  ...
  switch (funcode) {
    ...
    case OZ_IO_FS_PAGEREAD: {
      ...
      iopex -> req.u.readblocks.size = iopex -> u.pageread.p.blockcount * DISK_BLOCK_SIZE;
    ...
    case OZ_IO_FS_WILDSCAN: {
      ...
      if (sts == OZ_SUCCESS) sts = oz_knl_section_ugetz (procmode, sizeof iopex -> req.u.wildscan.wild, iopex -> u.wildscan.p.wild, iopex -> req.u.wildscan.wild, &wildlen);
      if ((sts == OZ_SUCCESS) && (iopex -> u.wildscan.p.wildsize != 0)) {
        if (wildlen > iopex -> u.wildscan.p.wildsize) wildlen = iopex -> u.wildscan.p.wildsize;
        sts = oz_knl_section_uput (procmode, wildlen, iopex -> req.u.wildscan.wild, iopex -> u.wildscan.p.wildbuff);
      }
      if (sts == OZ_SUCCESS) {
