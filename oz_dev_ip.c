...
#define ACCESSBUFFS_OFF oz_knl_process_setcur (oz_s_systemproc)					// turn off buffer access
												//   should be called before 
												//   oz_knl_iodone in case the 
												//   request being posted aborts 
												//   the process out
...
struct Tcpcon { Tcpcon *next;			/* next in tcpcons list */
                ...
                Iopex **tcprcv_iopqt;
                Iopex *poll_iopqh;		/* list of pending poll requests */
                Iopex **poll_iopqt;
...
struct Iopex { Iopex   *next;				/* general purpose 'next iopex' pointer */
               ...
               union { struct { OZ_IO_gen_poll       p; } genpoll;
                       struct { OZ_IO_ip_hwadd       p; char devname[OZ_DEVUNIT_NAMESIZE];     } hwadd;
...
static uLong be_genpoll (Iopex *iopex);
...
static void tcppolldone (Tcpcon *tcpcon, uLong mask);
...

{
  ...
  switch (funcode) {
    ...
    /* Generic poll request */

    case OZ_IO_GEN_POLL: {
      movc4 (as, ap, sizeof iopex -> u.genpoll.p, &(iopex -> u.genpoll.p));
      iopex -> be = be_genpoll;
      break;
    }
...

{
  ...
  while (1) {
    ACCESSBUFFS_OFF;							/* make sure we aren't attached to an old process */
    ...
    if (iopex != NULL) {
      ...
      if (sts != OZ_STARTED) {						/* maybe it finished */
        ACCESSBUFFS_OFF;
...

{
  ...
  /* See if it's already satisfied */

  alreadydone = 0;
  if (iopex -> u.genpoll.p.mask & OZ_IO_GEN_POLL_R) {
    if ((tcpcon -> rcvwindownxt != tcpcon -> rcvwindowins) || (tcpcon -> rcvdfin) || (tcpcon -> reset != 0)) {
      alreadydone |= OZ_IO_GEN_POLL_R;
    }
  }
  if (iopex -> u.genpoll.p.mask & OZ_IO_GEN_POLL_W) {
    transmitsize = tcpcon -> seq_lastrcvdwsize - tcpcon -> seq_nextusersendata;
    if (transmitsize > 0) alreadydone |= OZ_IO_GEN_POLL_W;
  }
  if (alreadydone != 0) return (OZ_POLLDONE + alreadydone);

  /* If not, queue it */

  *(tcpcon -> poll_iopqt) = iopex;
  iopex -> next = NULL;
  tcpcon -> poll_iopqt = &(iopex -> next);
  return (OZ_STARTED);
}
...

{
  ...
  tcpcon -> poll_iopqt             = &(tcpcon -> poll_iopqh); /* initialize poll request queue */
...

{
  ...
  /* If there are no data and there are immed receive requests, post them and get out */

  if ((tcpcon -> rcvwindownxt == tcpcon -> rcvwindowins) && !(tcpcon -> rcvdfin) && (tcpcon -> reset == 0)) {
    while (((iopex = tcpcon -> tcprcv_iopqh) != NULL) && (iopex -> u.tcpreceive.p.immed)) {
  ...
  /* There are some receive data */

  tcppolldone (tcpcon, OZ_IO_GEN_POLL_R);		/* there is data, complete any OZ_IO_GEN_POLL_R requests */
  ...
  /* Increment I/O chan reference count in case one of our calls to iodone ends */
  /* up deassigning the channel, the iochan (and chnex) won't be freed off      */
...

{
  ...
  /* Abort any TCP polls in progress */

  if (tcpcon != NULL) {
    for (liopex = &(tcpcon -> poll_iopqh); (iopex = *liopex) != NULL;) {
      if (abort -> deassign || oz_knl_ioabortok (iopex -> ioop, abort -> iochan, abort -> procmode, abort -> ioop)) {
        *liopex = iopex -> next;
        oz_knl_iodone (iopex -> ioop, OZ_ABORTED, NULL, NULL, NULL);
      } else {
...

{
  ...
  if ((tcpcon == NULL) && ((flags & (TCP_FLAGS_SYN | TCP_FLAGS_ACK)) == TCP_FLAGS_SYN)) { /* see if flags indicate and we don't already have one */
    ...
    *(tcplis -> u.tcplisten.prev) = tcplis -> next;
    ...
    /* The connection is now writeable */

    tcppolldone (tcpcon, OZ_IO_GEN_POLL_W);
  }
  ...
  if (flags & TCP_FLAGS_ACK) {
    ...
    if ((seqdiff < 0)  || ((seqdiff == 0) && (seqend == seq))) {
      ...
      /* Some new data was ackd, complete any OZ_IO_GEN_POLL_W requests */
...
/* Scan OZ_IO_GEN_POLL queue and post satisfied requests */
...

{
  ...
  for (liopex = &(tcpcon -> poll_iopqh); (iopex = *liopex) != NULL;) {
    if (!(iopex -> u.genpoll.p.mask & mask)) liopex = &(iopex -> next);
    else {
      *liopex = iopex -> next;
      ACCESSBUFFS_OFF;
      oz_knl_iodone (iopex -> ioop, OZ_POLLDONE + mask, NULL, NULL, NULL);
    }
  }
  tcpcon -> poll_iopqt = liopex;
}
...

{
  ...
  /* Terminate any pending polls */

  while ((iopex = tcpcon -> poll_iopqh) != NULL) {			/* get an IO POLL request */
    tcpcon -> poll_iopqh = iopex -> next; 				/* unlink it */
    oz_knl_iodone (iopex -> ioop, tcpcon -> reset, NULL, NULL, NULL);	/* abort each one */
  }
  tcpcon -> poll_iopqt = &(tcpcon -> poll_iopqh);			/* queue is now empty */
