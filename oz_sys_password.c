  ...
  for (i = 0; i < nitems; i ++) {
    ...
    switch (items[i].code) {
      case OZ_PASSWORD_CODE_PASSHASH: {
        ...
        else oz_sys_password_hashit (username, pwdrec_pw, items[i].size, items[i].buff);
...
void oz_sys_password_hashit (char const *username, char const *password, int hashsize, char *hashbuff)

{
  uByte hashbin[OZ_SYS_HASH_BINSIZE], hashctx[OZ_SYS_HASH_CTXSIZE];
  ...
  oz_sys_hash_init (hashctx);
  oz_sys_hash_data (hashctx, strlen (username), username);	// start out with username
  oz_sys_hash_data (hashctx, strlen (password), password);	// followed by password
  oz_sys_hash_term (hashctx, hashbin);
...
{
  char hashstr1[OZ_SYS_HASH_STRSIZE], hashstr2[OZ_SYS_HASH_STRSIZE], newpwhashstr[OZ_SYS_HASH_STRSIZE];
  char *oldstr, *p, pwdrec[RECORD_MAX], sepc;
  const char *username;
  int oldlen, si, username_l;
  OZ_Handle h_pwdfile;
  OZ_IO_fs_getinfo1 fs_getinfo1;
  OZ_IO_fs_readrec fs_readrec;
  uByte hashbin2[OZ_SYS_HASH_BINSIZE];
  uLong pwdrec_l, sts;
  ...
  /* Get what's in password record to hashstr2 */
  ...
  oldlen = p - oldstr;
  if ((oldlen != OZ_SYS_HASH_STRSIZE - 1) || !oz_sys_hash_str2bin (oldstr, hashbin2)) {
    *p = 0;
    oz_sys_password_hashit (username, oldstr, sizeof hashstr2, hashstr2);
    *p = sepc;
  } else {
    memcpy (hashstr2, oldstr, OZ_SYS_HASH_STRSIZE - 1);
  }

  /* Get what caller gave us for old password to hashstr1 */

  oz_sys_password_hashit (username, oldpw, sizeof hashstr1, hashstr1);

  /* See if they match */

  sts = OZ_BADPASSWORD;
  if (memcmp (hashstr1, hashstr2, OZ_SYS_HASH_STRSIZE - 1) != 0) goto rtnsts;

  /* Make new password hash string */

  oz_sys_password_hashit (username, newpw, sizeof newpwhashstr, newpwhashstr);
