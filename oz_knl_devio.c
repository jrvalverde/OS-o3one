...
struct OZ_Ioop { OZ_Objtype objtype;				/* object type OZ_OBJTYPE_IOOP */
                 ...
                 void (*finentry) (void *finparam, uLong *status_r);
                 void *finparam;
...
/************************************************************************/
/*									*/
/*  Set channel's processor mode					*/
/*									*/
/************************************************************************/

void oz_knl_iochan_setprocmode (OZ_Iochan *iochan, OZ_Procmode procmode)
...

{
  ...
  if (ioop -> state == IOOP_STATE_PSYNCMP) {
    if (ioop -> finentry != NULL) (*(ioop -> finentry)) (ioop -> finparam, &(ioop -> status));
    sts = ioop -> status;
  }
...
void oz_knl_iodonehi (OZ_Ioop *ioop, uLong status, void *kthread, void (*finentry) (void *finparam, uLong *status_r), void *finparam)
...
void oz_knl_iodone (OZ_Ioop *ioop, uLong status, void *kthread, void (*finentry) (void *finparam, uLong *status_r), void *finparam)
...

{
  ...
  if ((status_r != NULL) || (ioop -> finentry != NULL)) {
    ...
    if (ioop -> finentry != NULL) {
      (*(ioop -> finentry)) (ioop -> finparam, &(ioop -> status));					/* call the finentry routine */
    ...
    if (status_r != NULL) {
      ...
      if (ioop -> procmode == OZ_PROCMODE_KNL) *status_r = ioop -> status;				/* if caller is kernel mode, do it quickly */
        ...
        if (sts != OZ_SUCCESS) {
          oz_knl_printk ("oz_knl_iodone: error %u writing status block at %p\n", sts, status_r);
        }
      }
      ioop -> status_r = NULL;
    }
