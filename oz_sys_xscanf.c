...

{
  ...
  while ((fmtc = *(fmt ++)) != 0) {
    ...
    if (skip_space || (fmtc != 'c' && fmtc != 'C' && fmtc != 'n' && fmtc != '[')) {
      do {
        c = GETCH;
        if (c == EOF) goto input_error;
      } while (ISSPACE (c));
    ...
    switch (fmtc) {
      ...
      /* Match nonempty sequence of chars from the specified set */
      ...
      case '[': {
        ...
        include = 1;								/* assume set indicates what is included */
        if (*fmt == '^') { include = 0; fmt ++; }
        memset (masks, 0, sizeof masks);					/* clear masks indicating what to match on */
        do {
          c = *(fmt ++) & 0xFF;							/* get a char to be matched */
          masks[c/32] |= 1 << (c & 31);						/* set corresponding bit */
          if ((fmt[0] == '-') && (fmt[1] != ']')) {				/* check for range */
            while ((++ c <= (fmt[1] & 0xFF)) && (c < 256)) masks[c/32] |= 1 << (c & 31); /* if so, include all incl end char */
            fmt += 2;
          }
        } while (*fmt != ']');							/* repeat until we get a close bracket */
        fmt ++;									/* skip over close bracket */
        while ((c = GETCH) != EOF) {						/* get input stream character */
          if ((masks[c/32] & (1 << (c & 31))) ^ include) break;			/* stop if it doesn't match */
          if ((width >= 0) && (-- width <= 0)) break;				/* also stop if field width runs out */
          if (str != NULL) *(str ++) = c;					/* matches, copy to output buffer */
        }
        UNGETCH;								/* put non-matching character back */
        if (str != NULL) { *str = 0; done ++; }					/* null-terminate result string */
        break;
      }
