...
static void anal_abort (void *cp, OZ_Sigargs *sigargs, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx)
...
{
  Long cpuidx;
  OZ_Mchargx mchargx, *mchargxp;

  /* Figure out which cpu is current */

  cpuidx = anal_getcur (cp);

  /* Read the pointer to the mchargx struct for the current cpu */

  if (!readvirtmem_sys (&mchargxp, sizeof mchargxp, (OZ_Pointer)(crasheader -> mchargx + cpuidx))) return (0);

  /* Read the mchargx struct for the current cpu */

  if (!readvirtmem_sys (&mchargx, sizeof mchargx, (OZ_Pointer)mchargxp)) return (0);

  /* Using the current cpu's CR3, read the requested virtual memory */

  return (readvirtmem_cr3 (buff, size, (OZ_Pointer)addr, mchargx.cr3));
}
...
{
  return (readvirtmem_ptbr (buff, size, va, crasheader -> mchargx_cpy.ptbr_ro));
}
...

{
  ...
  return (readvirtmem_ptbr (buff, size, (OZ_Pointer)addr, mchargx.ptbr_ro));
}
...

{
  ...
  /* We should be able to read the corresponding mchargx pointer, too */

  if (!readvirtmem_sys (&mchargx, sizeof mchargx, (OZ_Pointer)(crasheader -> mchargx + cpuidx))) {
    oz_sys_io_fs_printf (oz_util_h_error, "%s: can't read mchargx ptr[%d] from %p\n", pn, cpuidx, crasheader -> mchargx + cpuidx);
    return (OZ_FILECORRUPT);
  }
  ...
  oz_sys_debug_halted (prompt, debugctx, mchargs, mchargx);
  return (OZ_SUCCESS);
}
...

{
  ...
  if (!anal_readmem (crasheader, &mchargx, sizeof mchargx, crasheader -> mchargx + crashcpu)) {
    oz_sys_io_fs_printf (oz_util_h_error, "%s: can't read mchargx ptr[%d] from %p\n", pn, crashcpu, crasheader -> mchargx + crashcpu);
    goto badheader;
  }
  ...
  oz_sys_debug_exception (prompt, debugctx, sigargs, mchargs, mchargx);
