...
struct OZ_Process { OZ_Objtype objtype;			/* OZ_OBJTYPE_PROCESS */
                    ...
                    uLong threadqseq;			/* sequence incremented each time new thread created */
...
OZ_Thread **oz_knl_process_getthreadqp (OZ_Process *process, int ifnormal, int inctqseq)

{
  if (process == NULL) process = getcur ("oz_knl_process_getthreadqp");
  OZ_KNL_CHKOBJTYPE (process, OZ_OBJTYPE_PROCESS);
  if (ifnormal && (process -> hwctx_state != HWCTX_STATE_NORMAL)) return (NULL);
  process -> threadqseq += inctqseq;
  if (process -> threadqseq == 0) process -> threadqseq = 1;
  return (&(process -> threadq));
}

uLong oz_knl_process_getthreadqseq (OZ_Process *process)

{
  if (process == NULL) process = getcur ("oz_knl_process_getthreadqp");
  OZ_KNL_CHKOBJTYPE (process, OZ_OBJTYPE_PROCESS);
  return (process -> threadqseq);
}

/************************************************************************/
/*									*/
...

{
  ...
  for (i = 0; i < nsections; i ++) {
    ...
    if (npages == 0) oz_crash ("oz_knl_process_mapsection: section has npages zero\n");
    if (npages + mapsecparams[i].svpage < npages) {
      oz_knl_printk ("oz_knl_process_mapsections*: npages %X at vpage %X\n", npages, mapsecparams[i].svpage);
      return (OZ_SECMAPADDRWRAPS);
    }
...

{
  ...
  if (pagetable == NULL) {
    oz_knl_printk ("oz_knl_process_mapsection*: no pt to map pages at 0x%X, process %p (%s)\n", vpage, process, process -> name);
    for (pagetable = process -> pagetables; pagetable != NULL; pagetable = pagetable -> next) {
      oz_knl_printk ("oz_knl_process_mapsection*:   0x%X page pt at 0x%X\n", pagetable -> maxpages, pagetable -> basepage);
    }
  }
  return (pagetable);
}
