...

{
  ...
  OZ_Password_item pwitems[6] = { OZ_PASSWORD_CODE_SECKEYSP,       sizeof seckeysbuff,  &seckeysbuff,  &seckeyssize, 
  ...
  *user_r = NULL;
  ...
  index = -1;
  sts   = oz_sys_password_getbyusername (username, 6, pwitems, &index, secmalloc, NULL);
  ...
  *em_r = "creating quota block";
  ...
  sts = oz_knl_user_create (username, quota, seckeys, maxbasepri, defdirbuff, &user); /* create the block (in case not already logged in) */
...
rtnsts:
  ...
  if ((index >= 4) && (defdirbuff   != NULL)) OZ_KNL_NPPFREE (defdirbuff);
  return (sts);
}
