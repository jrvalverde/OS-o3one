...
struct OZ_Section { OZ_Objtype objtype;			/* OZ_OBJTYPE_SECTION */
                    ...
                    OZ_Dbn hiblock;			/* number of total blocks in file */
...

{
  ...
  OZ_Mempage i, gpas;
  OZ_Section *section;
  uLong blocksperpage, l2blocksize, sts;
  ...
  *section_r = NULL;
  ...
  if (!(sectype & OZ_SECTION_TYPE_ZEROES)) {
    sts = OZ_VBNZERO;					/* virtual block numbers start at 1 */
    if (vbnstart == 0) goto err;
    ...
    if (vbnstart > fs_getinfo1.hiblock) goto err;
    blocksperpage = 1 << (OZ_HW_L2PAGESIZE - l2blocksize);
    i = (fs_getinfo1.hiblock - vbnstart + blocksperpage) >> (OZ_HW_L2PAGESIZE - l2blocksize);
							/* number of pages in file starting at vbnstart (rounding up) */
    if (i == 0) goto err;
  ...
  if (!(sectype & OZ_SECTION_TYPE_ZEROES)) {
    ...
    section -> hiblock   = fs_getinfo1.hiblock;		/* total blocks in file */
  }
...

{
  ...
  OZ_KNL_CHKOBJTYPE (oldsection -> file, OZ_OBJTYPE_IOCHAN);
...
lockoldpt:
    ...
    if (ptevaddr != NULL) {
      ...
      sts = oz_knl_section_faultpage (OZ_PROCMODE_KNL, OZ_HW_VADDRTOVPAGE (ptevaddr), 0, NULL);
      if (sts != OZ_SUCCESS) {
    ...
    while ((ptevaddr = oz_hw_pte_readany (vpage, &xpagestate, &xphypage, &xcurprot, &xreqprot)) != NULL) { /* make sure the pt we want is in */
      ...
      sts = oz_knl_section_faultpage (OZ_PROCMODE_KNL, OZ_HW_VADDRTOVPAGE (ptevaddr), 0, NULL); /* it's not in, fault it in */
      if (sts != OZ_SUCCESS) {
...

{
  ...
  while (bc != 0) {					/* repeat while more to do */
    while (!OZ_HW_READABLE (1, sp, procmode)) {		/* see if source readable by given access mode */
      ...
      sts = oz_knl_section_faultpage (procmode, OZ_HW_VADDRTOVPAGE (sp), 0, NULL); /* fault it in */
...

{
  ...
  while (bc != 0) {					/* repeat while more to do */
    while (!OZ_HW_WRITABLE (1, dp, procmode)) {		/* see if destination writable by given access mode */
      ...
      sts = oz_knl_section_faultpage (procmode, OZ_HW_VADDRTOVPAGE (dp), 1, NULL); /* fault it in */
      if (sts != OZ_SUCCESS) return (sts);		/* if it can't fault it in, return error status */
      pt = oz_knl_process_lockpt (process);		/* maybe it is faulted in now, try again */
    }
...
scanit:
  ...
  lastseclock = oz_knl_process_seclocks (process, npages, svpage);
  if (lastseclock == NULL) {
    oz_knl_printk ("oz_knl_section_iolock*: can't find secmap for 0x%x pages at 0x%x\n", npages, svpage);
    oz_knl_process_unlkpt (process, pt);
    OZ_KNL_NPPFREE (seclock);
    return (OZ_ACCVIO);
  }
  ...
  for (i = 0; i < npages; i ++) {							/* scan each page in question */
    if ((oz_hw_pte_readany (svpage + i, &pagestate, seclock -> a.phypages + i, &curprot, &reqprot) != NULL) /* see if it is accessible */
     || !protable[curprot][procmode][writing]) {
      oz_knl_process_unlkpt (process, pt);						/* it's not, unlock page table */
      sts = oz_knl_section_faultpage (procmode, svpage + i, writing, NULL);		/* try to fault it in */
      if (sts == OZ_SUCCESS) goto scanit;						/* retry if it says to */
      OZ_KNL_NPPFREE (seclock);								/* hard failure, release block */
      return (sts);									/* return failure status */
    }
  ...
  if (ptevaddr != NULL) {
    ...
    if (!itsadzvaddr (ptevaddr) 
     || (oz_knl_process_getsecfromvpage (process, 
      ...
      sts = oz_knl_section_faultpage (OZ_PROCMODE_KNL, OZ_HW_VADDRTOVPAGE (ptevaddr), 1, NULL);
...
start:
  ...
  while (npages > 0) {									/* repeat while there are pages to do */
    ...
    if (initsec != NULL) {
      if (pagestate != OZ_SECTION_PAGESTATE_PAGEDOUT) oz_crash ("oz_knl_section_setpageprot: vpage %x initial pagestate %d (%p)", svpage, pagestate, oz_hw_getrtnadr (1));
...
uLong oz_knl_section_faultpage (OZ_Procmode procmode, OZ_Mempage vpage, int writing, OZ_Mchargs *mchargs)
...
restart:
  ...
  if (oz_knl_process_getsecfromvpage (process, vpage, &section, &secpage, &defprot, &ownmode, &mapsecflags) == 0) {
    oz_knl_printk ("oz_knl_section_faultpage*: no section at vpage %X\n", vpage);
//    oz_knl_process_dump_secmaps (process);
  ...
  if (!protable[reqprot][procmode][writing]) {
    ...
    oz_knl_printk ("oz_knl_section_faultpage*: - reqprot %u, procmode %u, writing %u, processid %u\n", 
                   reqprot, procmode, writing, oz_knl_process_getid (oz_knl_process_getcur ()));
    goto rtnaccvio;
  }
...

{
  ...
  /* Otherwise, allocate a page and read page in */
    ...
    if (pfpage == 0) {
      ...
      l2bpp = OZ_HW_L2PAGESIZE - section -> l2blocksize;	/* LOG2 (blocks per page) */
      fs_pageread.startblock = (secpage << l2bpp) + section -> vbnstart;
      fs_pageread.blockcount = 1 << l2bpp;			/* just one physical page */
      if (fs_pageread.blockcount + fs_pageread.startblock >= section -> hiblock) {
        oz_hw_phys_initpage (phypage, 0);			/* partial page, zero the physical page first */
        fs_pageread.blockcount = section -> hiblock + 1 - fs_pageread.startblock; /* ... and read what we have */
      }
    } else {
      readfile = section -> pagefile;				/* pfpage non-zero, read from page file */
      oz_crash ("oz_knl_section readprivatepage: don't have pagefile blocksize");
      fs_pageread.startblock = ((pfpage - 1) << l2bpp) + 1;
    }
    status = OZ_PENDING;
...

{
  ...
  /* Otherwise, allocate a page and read page in */
    ...
    if (pfpage == 0) {
      ...
      oz_crash ("oz_knl_section readglobalpage: don't have pagefile blocksize");
      fs_pageread.blockcount = 1 << l2bpp;			/* just one physical page */
    }
    status = OZ_PENDING;
    sts = oz_knl_iostart3 (1, NULL, readfile, OZ_PROCMODE_KNL, NULL, NULL, &status, section -> pfevent, 
...
tryagain:
  ...
  if (ptevaddr != NULL) {										/* see if page containg pte is faulted out */
    ...
    sts = oz_knl_section_faultpage (OZ_PROCMODE_KNL, ptevpage, 0, NULL);				/* fault it in, this may be an upper level page */
  ...
  /* If I/O controller or fixed ramdisk page, just mark pte invalid as there is no page to write out and free off */
  ...
  if (section -> objtype != OZ_OBJTYPE_SECTION) {
    oz_crash ("oz_knl_section_unmappage: vpage %X, section %p, objtype %X", vpage, section, section -> objtype);
  }
    ...
    case OZ_SECTION_PAGESTATE_PAGEDOUT: {
      ...
      if (phypage != 0) {
        ...
        if (section -> sectype & OZ_SECTION_TYPE_PAGTBL) {
          ...
          oz_knl_section_faultpage (OZ_PROCMODE_KNL, vpage, 1, NULL);
          oz_knl_process_lockpt (process);
          keptptlock = 0;
          goto tryagain;
        }
...
unmapglobal:
  oz_crash ("oz_knl_section_unmappage*: global sections untested!\n");
...

{
  ...
  l2bpp = OZ_HW_L2PAGESIZE - section -> l2blocksize;
  fs_pagewrite.startblock = (secpage << l2bpp) + section -> vbnstart;
  fs_pagewrite.blockcount = 1 << l2bpp;
  if (fs_pagewrite.blockcount + fs_pagewrite.startblock >= section -> hiblock) {
    fs_pagewrite.blockcount = section -> hiblock + 1 - fs_pagewrite.startblock; /* partial page, write what we can */
  }
  status = OZ_PENDING;
