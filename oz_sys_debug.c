...
static void command (OZ_Debug *dc, OZ_Sigargs *sigargs, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx, const char *prompt);
static char *getaddrexp (OZ_Debug *dc, char *cmdbuf, int *varsize, OZ_Pointer *varaddr, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx, const char *prompt);
static char *getexp (OZ_Debug *dc, char *cmdbuf, int *varsize, OZ_Pointer *varaddr, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx, const char *prompt);
static char *getoperand (OZ_Debug *dc, char *cmdbuf, int *varsize, OZ_Pointer *varaddr, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx, const char *prompt);
static void removebreaks (OZ_Debug *dc, const char *prompt);
static int disassemble (OZ_Debug *dc, OZ_Pointer varaddr, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx, OZ_Pointer *varsize_r);
static void traceback (OZ_Debug *dc, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx, int framecount);
static void printstring (OZ_Debug *dc, int outsize, int varsize, OZ_Pointer varaddr);
static void printaddr (OZ_Debug *dc, OZ_Pointer varaddr, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx);
...
int oz_sys_debug_exception (const char *prompt, OZ_Debug *dc, OZ_Sigargs *sigargs, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx)
...
haltothers:
  ...
  if ((sigargs != NULL) && (sigargs[1] == OZ_BREAKPOINT)) {	/* see if sigargs say we might have */
    for (i = 0; i < OZ_DEBUG_NBREAKS; i ++) {			/* ok, loop through table of known breakpoints */
      if (GETNXTINSAD (mchargi) == dc -> breakaddr[i]) {	/* see if the address matches (kernel has already backed it up) */
...
void oz_sys_debug_halted (const char *prompt, OZ_Debug *dc, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx)
...

{
  ...
  if ((varaddr >= (OZ_Pointer)mchargx) && (offs < sizeof *mchargx)) {
    for (i = 0; dc -> cb -> mchargxdes[i].name != NULL; i ++) {
