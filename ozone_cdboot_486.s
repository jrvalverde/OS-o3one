...
!
	int	0x12			! get base memory size in KB
	shl	ax,#6			! save top segment inclusive
	dec	ax			! ... so we don't trounce EBDA
	mov	basetopseg+LB,ax
...
readloop:
	...
	sub	cx,basetopseg+LB	! see if we've reached the hole
	jc	readloop		! repeat if not

	mov	ax,startlbn+0+LB	! get block to read
	mov	dx,startlbn+2+LB
	mov	cx,segment+LB		! get segment number
	mov	bx,basetopseg+LB	! see how many segments left to the hole
	sub	bx,cx
	jc	readdone
...
basetopseg:	.word	0x9FFF		! filled in with top segment number - 1
