...
struct Adevex { Long chansassnd;		/* count of channels assigned */
                ...
                Aiopex *genpolls;		/* list of pending OZ_IO_GEN_POLL iopex's */
...
struct Aiopex { Aiopex *next;
                ...
                union { struct { OZ_IO_gen_poll p;
                               } genpoll;
                        struct { int satisfied;			/* 0 : still waiting for it; 1 : got it */
...
static void iodone (Aiopex *app_iopex, uLong status, void (*finentry) (void *finparam, uLong *status_r), void *finparam);
static void ctrlcharfin (void *ccv, uLong *status_r);
static void write_iodone (void *app_iopexv, uLong *status_r);
static void read_iodone (void *app_iopexv, uLong *status_r);
static void console_kbd_char (void *app_devexv, char bl);
static void conpolldone (Adevex *app_devex, uLong mask);
...
static void fin_gotsetmode (void *app_iopexv, uLong *status_r);
...
abort_genpolls:
  kb = lockdb (app_devex, NULL);								/* lock the queue */
  for (liopex = &(app_devex -> genpolls); (iopex = *liopex) != NULL; liopex = &(iopex -> next)) { /* scan through queue */
    ...
    *liopex = iopex -> next;									/* abort, unlink from queue */
    unlkdb (app_devex, kb);									/* release the lock */
    iodone (iopex, OZ_ABORTED, NULL, NULL);							/* complete the request with aborted status */
    goto abort_genpolls;									/* see if any more to abort */
  }
...

{
  ...
  switch (funcode) {
    ...
    case OZ_IO_GEN_POLL: {
      movc4 (as, ap, sizeof app_iopex -> u.genpoll.p, &(app_iopex -> u.genpoll.p));
      kb  = lockdb (app_devex, ioop);				/* get access to interrupt level variables */
      sts = OZ_DEVOFFLINE;
      if (app_devex -> online) {				/* make sure device is online */
        sts = 0;
        if ((app_iopex -> u.genpoll.p.mask & OZ_IO_GEN_POLL_R) && (app_devex -> rahin != app_devex -> rahout)) sts |= OZ_IO_GEN_POLL_R;
        if ((app_iopex -> u.genpoll.p.mask & OZ_IO_GEN_POLL_W) && !(app_devex -> output_suspend) && (app_devex -> tt_status != OZ_PENDING)) sts |= OZ_IO_GEN_POLL_W;
        if (sts != 0) sts += OZ_POLLDONE;
        else {
          app_iopex -> next = app_devex -> genpolls;		/* put this request on genpolls list, last-in/first-out order */
          app_devex -> genpolls = app_iopex;
          sts = OZ_STARTED;
        }
      }
      unlkdb (app_devex, kb);
    ...
    case OZ_IO_CONSOLE_SETMODE: {
      ...
      if (sts == OZ_SUCCESS) {
        ...
        if (sts != OZ_STARTED) fin_gotsetmode (app_iopex, &sts); /* if synchronous completion, copy out result */
      }
      return (sts);
    }
...

{
  ...
  if (iopex -> u.write.wlen != NULL) *(iopex -> u.write.wlen) = iopex -> write_done;
}
...

{
  ...
  if (bl == 17) {								/* check for ctrl-q */
    ...
    if (app_devex -> tt_status != OZ_PENDING) conpolldone (app_devex, OZ_IO_GEN_POLL_W);
...
check_rah:
  ...
  /* Complete any poll read requests */

  conpolldone (app_devex, OZ_IO_GEN_POLL_R);
...

{
  ...
  for (liopex = &(app_devex -> genpolls); (iopex = *liopex) != NULL;) {
    if (!(iopex -> u.genpoll.p.mask & mask)) liopex = &(iopex -> next);
    else {
      *liopex = iopex -> next;
      oz_knl_iodonehi (iopex -> ioop, OZ_POLLDONE + mask, NULL, NULL, NULL);
    }
  }
}

...

{
  ...
  if (sts != OZ_STARTED) {
    ...
    app_devex -> tt_status = OZ_SUCCESS;
    if (!(app_devex -> output_suspend)) conpolldone (app_devex, OZ_IO_GEN_POLL_W);
  }
...

{
  ...
  sts = disp_start_flush (app_devex);
  if ((sts != OZ_STARTED) && (app_devex -> tt_status == OZ_SUCCESS)) app_devex -> tt_status = sts;
  if (!(app_devex -> output_suspend) && (app_devex -> tt_status != OZ_PENDING)) conpolldone (app_devex, OZ_IO_GEN_POLL_W);
...

{
  ...
  movc4 (sizeof app_iopex -> u.gsmode.modebuff, &(app_iopex -> u.gsmode.modebuff), 
