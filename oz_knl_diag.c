...
static uLong reboot_thread (void *dummy);
...
read_command:
  ...
  if (strcmp (conbuf, "3") == 0) {
    sts = oz_knl_thread_create (oz_s_systemproc, -1, NULL, NULL, NULL, 
                                0, reboot_thread, NULL, OZ_ASTMODE_INHIBIT, 
                                25, "oz_knl_diag reboot_thread", NULL, &thread);
    if (sts == OZ_SUCCESS) return;
    oz_knl_printk ("error %u creating reboot thread\n", sts);
    goto read_command;
  }
...
copy_doney:
  ...
  if (strcmp (conbuf, "13") == 0) reboot_thread (NULL);
...
/************************************************************************/
/*									*/
/*  This runs as a thread to reboot the computer			*/
/*									*/
/************************************************************************/
...

{
  oz_hw_cpu_setsoftint (0);
  oz_knl_shutdown ();
  oz_knl_printk ("oz_knl_diag: rebooting...\n");
...
#else
  entrypoint = startaddr;
  oz_sys_printkp (0, "oz_knl_diag thread_init: calling entrypoint %p\n", entrypoint);
  exsts = (*entrypoint) (NULL);
#endif
...

{
  ...
  oz_knl_printk ("oz_knl_halt: cpu %d halting (%X)\n", cpuidx, oz_hw_cpu_smplevel ());	// print a message
