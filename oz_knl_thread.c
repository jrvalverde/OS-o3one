...
#define HS_DETACHED  0	// no debugger attached to thread (must be 0 as it is default/initial state)
#define HS_ATTACHING 1	// debugger in process of attaching
#define HS_ATTACHED  2	// debugger attached to thread; thread is running
#define HS_HALTED    3	// debugger attached to thread; thread is halted
...
struct OZ_Thread { OZ_Objtype objtype;				/* OZ_OBJTYPE_THREAD */
                   ...
                   struct {
                     Long volatile state;			/* HS_... */
                     uLong (*entry) (void *param, OZ_Procmode procmode, OZ_Sigargs const *sigargs, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx);
                     void *param;
                   } halts[OZ_PROCMODE_MAX-OZ_PROCMODE_MIN+1];	/* debugger halt info */
                   ...
                   uQuad hw_ctx[(OZ_THREAD_HW_CTX_SIZE+sizeof(uQuad)-1)/sizeof(uQuad)]; /* hardware context data */
...
static void haltedast (void *astparam, uLong aststs, OZ_Mchargs *mchargs);
...

{
  ...
  threadqp = oz_knl_process_getthreadqp (process, ifnormal, 1);			/* get threadqp = &(process -> threadq) */
  if (threadqp == NULL) {
...

{
  ...
  if (thread -> initevent != NULL) {				/* see if an initial event flag was specified for the thread */
    eventlist[0].event = thread -> initevent;			/* wait for it to be set */
    while (1) {
      oz_hw_cpu_setsoftint (0);
      sts = oz_knl_event_waitlist (1, eventlist, OZ_PROCMODE_KNL, 1);
      if (sts != OZ_ASTDELIVERED) break;
      oz_hw_cpu_setsoftint (1);					/* but allow kernel mode ast's (like abort) to be processed */
    }
    if ((sts != OZ_FLAGWASSET) && (sts != OZ_FLAGWASCLR)) oz_crash ("oz_knl_thread_start*: init event wait status %u", sts);
...
uLong oz_knl_exhand_create (OZ_Exhand_entry exhentry, void *exhparam, OZ_Procmode procmode, OZ_Thread *thread)
...
void oz_knl_exhand_delete (OZ_Exhand_entry exhentry, void *exhparam, OZ_Procmode procmode, OZ_Thread *thread)

{
  ...
  tp = oz_hw_smplock_wait (&(thread -> smplock_tp));
  for (lexhand = &(thread -> exhandq[procmode]); (exhand = *lexhand) != NULL;) {
    if ((exhentry != NULL) && (exhand -> exhentry != exhentry)) goto skipit;
    if ((exhparam != NULL) && (exhand -> exhparam != exhparam)) goto skipit;
    *lexhand = exhand -> next;
...

{
  ...
  oz_hw_cpu_setsoftint (1);					// kernel mode ast's (and all ast's for that matter) 
								// ... execute with softint delivery enabled
  checkknlastq (thread, NULL);					// execute them
  oz_hw_cpu_setsoftint (0);					// inhibit softints again for what's to come

  /* Get rid of any attaches.  With 'knlexit' set, no new ones should appear. */

  for (pm = OZ_PROCMODE_MAX; pm >= OZ_PROCMODE_MAX; -- pm) {
    oz_knl_thread_halted (pm, (OZ_Sigargs const *)1, NULL, NULL);
  }
  ...
  while (1) {
    ...
    for (zthread = *oz_knl_process_getthreadqp (process, 0, 0); zthread != NULL; zthread = zthread -> proc_next) { /* loop through the list of threads */
...
going_le_zero:
  ...
  *(thread -> siblingprev) = sibling = thread -> siblingnext;	/* remove it from orphan list */
  ...
  threadqp = oz_knl_process_getthreadqp (process, 0, 0);	/* remove it from process' list of threads */
  thread -> process = NULL;
...
waitloop:
  ...
  while ((thread -> state != OZ_THREAD_STATE_RUN) 			// maybe a hero will come along and put us right back in RUN
    ...
    if (oz_knl_lowipl_lowipls != NULL) {				// also, if there are any pending lowipl routines ...
      OZ_HW_WAITLOOP_TERM;						// ... enable hw interrupts, inhibit softint delivery
      oz_knl_lowipl_handleint ();					//     process them
      OZ_HW_WAITLOOP_INIT;						//     inhibit all interrupt delivery
      continue;								//     maybe an lowipl routine woke us up or something
    }
    OZ_HW_WAITLOOP_BODY;						// nothing to do, enable all ints, wait for int, inhib ints
  }
  OZ_HW_WAITLOOP_TERM;							// enable hw interrupts, inhibit softint delivery
...
uLong oz_knl_thread_attach (OZ_Thread *thread, OZ_Procmode procmode, uLong (*entry) (void *param, OZ_Procmode procmode, OZ_Sigargs const *sigargs, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx), void *param)

{
  ...
  sts = OZ_SUCCESS;							/* assume we will be able to attach */
  tp = oz_hw_smplock_wait (&(thread -> smplock_tp));			/* set smp lock to lock the exit handler lists */
  if (thread -> knlexit) sts = OZ_THREADEAD;				/* can't if thread has exited already */
  else if (!oz_hw_atomic_setif_long (&(thread -> halts[procmode-OZ_PROCMODE_MIN].state), HS_ATTACHING, HS_DETACHED)) sts = OZ_THREADATTACHED;
  else {
    thread -> halts[procmode-OZ_PROCMODE_MIN].entry = entry;		/* ok, mark thread 'attached' */
    thread -> halts[procmode-OZ_PROCMODE_MIN].param = param;
    OZ_HW_MB;
    thread -> halts[procmode-OZ_PROCMODE_MIN].state = HS_ATTACHED;
  }
  oz_hw_smplock_clr (&(thread -> smplock_tp), tp);			/* if knlexit gets set, the entry routine will get called */
...
/************************************************************************/
/*									*/
/*  Cause a thread to halt						*/
/*									*/
/*    Input:								*/
/*									*/
/*	thread   = target thread to halt				*/
/*	procmode = processor mode to halt it at				*/
/*	smplevel = softint						*/
/*									*/
/*    Output:								*/
/*									*/
/*	oz_knl_thread_halt = OZ_SUCCESS : halt queued			*/
/*	                           else : error status			*/
/*									*/
/*    Note:								*/
/*									*/
/*	This causes the target to signal an OZ_HALTED exception at the 	*/
/*	indicated processor mode (in an express ast).  It is assumed 	*/
/*	someone has 'attached' the thread for debugging and the 	*/
/*	debugger will see the OZ_HALTED exception.			*/
/*									*/
/*	If a debugger is not attached, the thread will see the 		*/
/*	OZ_HALTED exception and it will process it, most likely by 	*/
/*	aborting.							*/
/*									*/
/************************************************************************/

uLong oz_knl_thread_halt (OZ_Thread *thread, OZ_Procmode procmode)

{
  ...
  /* Queue an halt ast, causing it to call 'haltedast' asap */

  sts = oz_knl_ast_create (thread, procmode, haltedast, NULL, 1, &ast);
  if (sts == OZ_SUCCESS) oz_knl_thread_queueast (ast, OZ_HALTED);
  return (sts);
}

/* Called in target thread's context at outer mode, signal an OZ_HALTED exception */

static void haltedast (void *astparam, uLong aststs, OZ_Mchargs *mchargs)

{
  static OZ_Sigargs const halted_sigargs[3] = { 2, OZ_HALTED, 0 };

  if (aststs == OZ_HALTED) oz_sys_condhand_call (halted_sigargs, mchargs);
}

/************************************************************************/
/*									*/
/*  This routine will post the debugging call for the calling thread	*/
/*									*/
/*  This routine is called at the very beginning of 			*/
/*  oz_sys_condhand_call so a debugger can trap all exceptions		*/
/*									*/
/*  It is also called in the exit handling to tell debugger the target 	*/
/*  is exiting								*/
/*									*/
/*    Input:								*/
/*									*/
/*	procmode = processor mode of the exception			*/
/*	sigargs  = exception signal arguments (NULL in exit code)	*/
/*	mchargs  = exception standard machine arguments (NULL in exit code)
/*	mchargx  = exception extended machine arguments (NULL in exit code)
/*	           [0] = values						*/
/*	           [1] = modification mask				*/
/*	smplevel = softint						*/
/*									*/
/*    Output:								*/
/*									*/
/*	oz_knl_thread_halted = OZ_HALTED : thread is not under control of debugger
/*	                     OZ_RESIGNAL : call thread's condition handler
/*	                       OZ_RESUME : resume execution after signal
/*									*/
/*    Note:								*/
/*									*/
/*	sigargs and mchargs/x are in outer mode's addressing space and 	*/
/*	have not been probed/locked					*/
/*									*/
/************************************************************************/

uLong oz_knl_thread_halted (OZ_Procmode procmode, OZ_Sigargs const *sigargs, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx)

{
  OZ_Astmode saveastmode;
  OZ_Thread *thread;
  uLong rc;

  /* Make sure there is a debugger attached and mark thread halted */

  thread = oz_knl_thread_getcur ();
  if (!oz_hw_atomic_setif_long (&(thread -> halts[procmode-OZ_PROCMODE_MIN].state), HS_HALTED, HS_ATTACHED)) return (0);

  /* Don't let it receive ast's at current or outer levels while halted so it appears truly halted */

  saveastmode = thread -> astmode[procmode-OZ_PROCMODE_MIN];
  thread -> astmode[procmode-OZ_PROCMODE_MIN] = OZ_ASTMODE_INHEXP;

  /* Call the callback and let it decide what to do with thread                */
  /* It can remain attached or can decide to detach                            */
  /* It can either continue from the exception or let thread's handlers see it */

  rc = (*(thread -> halts[procmode-OZ_PROCMODE_MIN].entry)) (thread -> halts[procmode-OZ_PROCMODE_MIN].param, procmode, sigargs, mchargs, mchargx);

  /* Restore ast mode */

  thread -> astmode[procmode-OZ_PROCMODE_MIN] = saveastmode;

  /* If exiting (ie, sigargs==NULL), make sure they are detaching */

  if (!(rc & OZ_THREAD_HALT_LOOPBK) && (sigargs == NULL) && (rc & OZ_THREAD_HALT_ATTACH)) {
    oz_crash ("oz_knl_thread_halted: did not detach on exit");
  }

  /* sts<1> set : remain attached; clear : detach */

  thread -> halts[procmode-OZ_PROCMODE_MIN].state = (rc & OZ_THREAD_HALT_ATTACH) ? HS_ATTACHED : HS_DETACHED;

  /* sts<0> set : resume; clear : resignal exception                             */
  /* sts<2> set : just loopback after returning to outer mode (to deliver ast's) */
...

{
  ...
  if (thread == NULL) thread = *oz_knl_process_getthreadqp (process, 0, 0);
...

{
  ...
  count = 0;
  ps = oz_knl_process_lockps (process);
  for (thread = *oz_knl_process_getthreadqp (process, 0, 0); thread != NULL; thread = thread -> proc_next) count ++;
  oz_knl_process_unlkps (process, ps);
  return (count);
}

...

{
  ...
  ps = oz_knl_process_lockps (process);
  for (thread = *oz_knl_process_getthreadqp (process, 0, 0); thread != NULL; thread = thread -> proc_next) oz_knl_thread_dump (thread);
  oz_knl_process_unlkps (process, ps);
}
