...
scalefactor:		.long	0		# oz_TIMER_RESOLUTION / rdtscpersec * 2^scaleshift
scaleshift:		.long	0
unscalefactor:		.long	0		# rdtscpersec / oz_TIMER_RESOLUTION * 2^unscaleshift
unscaleshift:		.long	0
...
## Calculate basetime = what time it was when the RDTSC was zero
	...
	movl	basetime+4,%edx
	call	timer_scale			# convert it to 100nS ticks in %eax:%ebx

	movl	oz_s_boottime+0,%ecx		# subtract from boottime
	movl	oz_s_boottime+4,%edx

	subl	%ebx,%ecx			# ... to get the basetime
	sbbl	%eax,%edx

	movl	%ecx,basetime+0
	movl	%edx,basetime+4

	call	oz_hw_tod_iotanow		# init lastrdtsc and this CPU's CPU_Q_RDTSCBIAS 
...
oz_hw486_asys2iota:
	...
	# %eax:%ebx = rdtsc value, 0xFFFFFFFF:FFFFFFFF if overflow
...
oz_hw_tod_setrate:
	...
	movl	%ebx,%esi				# save scaled result
	movl	%eax,%edi
	...
	call	timer_scale
	subl	%ebx,%esi				# this is how much to add to basetime
	sbbl	%eax,%edi
...
##########################################################################
##									##
##  Scale number of rdtsc ticks in %edx:%eax to 100nS in %eax:%ebx	##
##    Scratch is %ecx, %edx						##
##    Preserves %esi, %edi						##
##									##
##########################################################################
...
timer_scale:
	...
	mull	scalefactor		# edx:eax = mid:low orders
	...
	mull	scalefactor		# multiply high order ticks by scalefactor
	...
	## %edx:%eax:%ebx = rdtsc_ticks * scalefactor

	movb	scaleshift,%cl		# see how many bits to shift right by
	testb	$32,%cl
	jz	timer_scale_shift
	movl	%eax,%ebx		# if ge 32, shift registers down
	movl	%edx,%eax
	xorl	%edx,%edx
timer_scale_shift:
	shrdl	%cl,%eax,%ebx		# shift %edx:%eax:%ebx right by %cl<4:0>
	shrdl	%cl,%edx,%eax
	shrl	%cl,%edx		# shift %edx just for overflow detection
	jz	timer_scale_rtn
	movl	$-1,%ebx
	movl	$-1,%eax
timer_scale_rtn:

	## %eax:%ebx = rdtsc_ticks * scalefactor >> scaleshift
...
oz_hw_tod_dsys2iota:
	pushl	%ebp		# make call frame
	movl	%esp,%ebp
	pushl	%ebx		# save scratch

	movl	 8(%ebp),%eax	# get system time
	movl	12(%ebp),%edx

	call	timer_unscale	# convert to rdtsc time

	movl	%eax,%edx	# put in correct return spot
	movl	%ebx,%eax
...
# Convert %edx:%eax from system ticks to rdtsc ticks in %eax:%ebx
# Scratch: %ecx,%edx

	.align	4
timer_unscale:
	movl	%edx,%ecx		# save high order ticks in %ecx
	mull	unscalefactor		# edx:eax = mid:low orders
	pushl	%edx			# save high order result on stack for now
	movl	%eax,%ebx		# save low order result in %ebx
	movl	%ecx,%eax		# get high order ticks back
	mull	unscalefactor		# multiply high order ticks by unscalefactor
	popl	%ecx			# restore intermediate order result
	addl	%ecx,%eax		# add in other intermediate order result
	adcl	$0,%edx			# add any carry to high order result

	## %edx:%eax:%ebx = rdtsc_ticks * unscalefactor

	movb	unscaleshift,%cl	# see how many bits to shift right by
	testb	$32,%cl
	jz	timer_unscale_shift
	movl	%eax,%ebx		# if ge 32, shift registers down
	movl	%edx,%eax
	xorl	%edx,%edx
timer_unscale_shift:
	...
	shrdl	%cl,%edx,%eax
	shrl	%cl,%edx		# shift %edx just for overflow detection
	jz	timer_unscale_rtn
	movl	$-1,%ebx
	movl	$-1,%eax
timer_unscale_rtn:

	## %eax:%ebx = rdtsc_ticks * unscalefactor >> unscaleshift

	ret

##########################################################################
##									##
##  Have new rdtscpersec, calculate various scaling factors		##
##									##
##    Input:								##
##									##
##	oz_hw486_rdtscpersec = RDTSC ticks per second			##
##									##
##    Output:								##
##									##
##	oz_hw486_pitfactor>>pitshift = PIT counts per RDTSC		##
##	scalefactor>>scaleshift = 10^7/rdtscpersec			##
##	unscalefactor>>unscaleshift = rdtscpersec/10^7			##
##									##
##    Scratch:								##
##									##
##	%eax,%ecx,%edx							##
...
##########################################################################
	...
	.align	4
have_new_rdtscpersec:
	pushl	%ebx
	movl	oz_hw486_rdtscpersec,%ebx

	# Calculate pitfactor>>pitshift = 1193182/rdtscpersec

	movl	$1193182,%eax
	call	calcfactorshift
	movl	%eax,oz_hw486_pitfactor
	movl	%ecx,oz_hw486_pitshift

	# Calculate scalefactor>>scaleshift = 10^7/rdtscpersec

	movl	oz_TIMER_RESOLUTION,%eax
	call	calcfactorshift
	movl	%eax,scalefactor
	movl	%ecx,scaleshift

	# Calculate unscalefactor>>unscaleshift = rdtscpersec/10^7

	movl	%ebx,%eax
	movl	oz_TIMER_RESOLUTION,%ebx
	call	calcfactorshift
	movl	%eax,unscalefactor
	movl	%ecx,unscaleshift

	popl	%ebx
	ret

	# Calculate eax>>ecx = eax / ebx

	.align	4
calcfactorshift:
	xorl	%ecx,%ecx
	xorl	%edx,%edx
calcscalefactor_loop:
	addl	%eax,%eax			# try doubling edx:eax
	adcl	%edx,%edx
	jc	calcscalefactor_over
	incl	%ecx				# and incrementing exponent
	cmpl	%ebx,%edx			# see if edx .ge. ebx yet
	jc	calcscalefactor_loop		# if not, try doubling again
	shrdl	$1,%edx,%eax			# halve edx:eax (undo the doubling)
	shrl	$1,%edx
	decl	%ecx				# undo incrementing exponent
calcscalefactor_finish:
	divl	%ebx				# divide eax=edx:edx/ebx r edx
	addl	%edx,%edx			# double the remainder
	jc	calcscalefactor_round
	cmpl	%edx,%ebx			# set carry iff divisor .lt. doubled remainder
calcscalefactor_round:
	adcl	$0,%eax
	sbbl	$0,%eax				# make sure rounding didn't overflow us out of business
	ret
calcscalefactor_over:
	shrdl	$1,%edx,%eax
	shrl	$1,%edx
	orl	$0x80000000,%edx
	jmp	calcscalefactor_finish

##########################################################################
##									##
