...
/* Use the OZ_SYS_PDATA macro to access the array                                                      */
...
/* These are macros to access oz_sys_pdata array */

	// this can be called from any mode

#define OZ_SYS_PDATA(__procmode) (((__procmode) == OZ_PROCMODE_SYS) ? &oz_s_systempdata : &(oz_sys_pdata_array[oz_hw_procmode_max(__procmode)-OZ_PROCMODE_MIN].data))

	// usable when it is known that the caller is in kernel mode accessing process local array (no OZ_PROCMODE_SYS)
