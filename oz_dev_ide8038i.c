...
static void atapi_reqdone (void *iopexv, uLong *status_r);
    ...
    config ()
    {
      ...
      if any drive(s) found, 
        ...
        probe_atadrive ()
        {
          ...
          send ATA identify command to see if ATA drive is there
...

{
  ...
  if (sts == 0) {
    oz_knl_printk ("oz_dev_ide8038i: - timed out resetting\n");
    return (0);
  }
  status = sts;							// check for error
  if ((status & 0xD9) != 0x50) {
    oz_knl_printk ("oz_dev_ide8038i: - reset status 0x%X\n", status);
    if (status & 0x01) {
