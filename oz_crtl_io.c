...
static int fsio_vioctl (int fd, int request, va_list ap);
static int fsio_close (int fd);

static const OZ_Crtl_fd_driver fsio_driver = { fsio_fxstat, fsio_lseek, fsio_read, fsio_write, fsio_vioctl, NULL, fsio_close };
...

{
  ...
  va_start (ap, cmd);
  if (oz_crtl_fd_array[fd].driver -> vfcntl == NULL) {
    oz_sys_io_fs_printerror ("oz_crtl_io fcntl*: (%d, %d, )\n", fd, cmd);	// ??
    return (0);
  }
  return ((*(oz_crtl_fd_array[fd].driver -> vfcntl)) (fd, cmd, ap));
}

...

{
  ...
  if (prot & PROT_WRITE) pageprot = OZ_SYS_PDATA (OZ_PROCMODE_KNL) -> rwpageprot;

  npagem = (len + PS - 1) >> OZ_HW_L2PAGESIZE;				/* get total number of pages to map */
  ...
  if (flags & MAP_ANON) {
    ...
    /* Create a demand-zero section */

    sts = oz_sys_section_create (OZ_PROCMODE_KNL, 0, npagem, 0, OZ_SECTION_TYPE_ZEROES, &h_section);
    if (sts != OZ_SUCCESS) {
      errno = EOZSYSERR;
      errno_ozsts = sts;
      return ((__ptr_t)-1);
  ...
  /* Mapping a file to memory */
    ...
    if (!oz_crtl_fd_check (fd)) return ((__ptr_t)-1);
    memset (&fs_getinfo1, 0, sizeof fs_getinfo1);				/* find out about file */
    sts = oz_sys_io (OZ_PROCMODE_KNL, oz_crtl_fd_array[fd].h_iochan, oz_crtl_fd_array[fd].h_event, 
                     OZ_IO_FS_GETINFO1, sizeof fs_getinfo1, &fs_getinfo1);
    if (sts != OZ_SUCCESS) {
    ...
    off = (off / fs_getinfo1.blocksize) + 1;				/* make offset into a VBN to start at */

    /* Create a file section */

    sts = oz_sys_section_create (OZ_PROCMODE_KNL, oz_crtl_fd_array[fd].h_iochan, npagem, off, 0, &h_section);
    if (sts != OZ_SUCCESS) {
      errno = EOZSYSERR;
      errno_ozsts = sts;
      return ((__ptr_t)-1);
  ...
  return (OZ_HW_VPAGETOVADDR (svpage));
}
...

{
  switch (request) {
    ...
    /* Get rows & columns on a terminal screen */
    ...
    case TIOCGWINSZ: {
      ...
      sts = oz_sys_io (OZ_PROCMODE_KNL, oz_crtl_fd_array[fd].h_iochan, oz_crtl_fd_array[fd].h_event, 
                       OZ_IO_CONSOLE_GETMODE, sizeof console_getmode, &console_getmode);
      if (sts == OZ_SUCCESS) {
        ws = va_arg (ap, struct winsize *);
        memset (ws, 0, sizeof *ws);
        ws -> ws_row = console_modebuff.rows;
        ws -> ws_col = console_modebuff.columns;
        return (0);
      }
      if (sts == OZ_BADIOFUNC) errno = ENOTTY;
      else {
        errno = EOZSYSERR;
        errno_ozsts = sts;
      }
...

{
  ...
  termios_p -> c_iflag = IGNBRK | IGNPAR | ICRNL | IXON | IXOFF | IMAXBEL;
  termios_p -> c_oflag = ONLCR;
  termios_p -> c_cflag = CS8;
  termios_p -> c_lflag = ICANON | ECHO;
  termios_p -> c_cc[VINTR]    = 0177;
  termios_p -> c_cc[VQUIT]    =  034;
  termios_p -> c_cc[VERASE]   =  010;
  termios_p -> c_cc[VKILL]    =  025;
  termios_p -> c_cc[VEOF]     =  004;
  termios_p -> c_cc[VSTART]   =  021;
  termios_p -> c_cc[VSTOP]    =  023;
  termios_p -> c_cc[VSUSP]    =  032;
  termios_p -> c_cc[VLNEXT]   =  026;
  termios_p -> c_cc[VWERASE]  =  027;
  termios_p -> c_cc[VREPRINT] =  022;
  termios_p -> c_cc[VDISCARD] =  017;
