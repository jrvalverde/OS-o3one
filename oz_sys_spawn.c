...
uLong oz_sys_spawn (OZ_Handle h_job, 
                    const char *image, 
                    OZ_Handle h_input, 
                    OZ_Handle h_output, 
                    OZ_Handle h_error, 
...
{
  return (oz_sys_spawn2 (h_job, image, h_input, h_output, h_error, h_init, h_exit, 
                         defdir, 0, nparams, paramv, name, thread_h_r, process_h_r));
}

OZ_HW_SYSCALL_DEF_14 (spawn2, OZ_Handle, h_job, 
                              const char *, image, 
                              OZ_Handle, h_input, 
                              OZ_Handle, h_output, 
                              OZ_Handle, h_error, 
...

{
  ...
  OZ_Job *job;
  OZ_Logname *logname_debug, *logname_defdir, *logname_deftables, *logname_error, *logname_image, *logname_imgdir, *logname_input, *logname_output, *logname_params, *logname_parent;
  ...
  /* If debugging, create logical to enable debugger */

  if (flags & OZ_SYS_SPAWN_FLAG_DEBUG) {
    sts = oz_knl_logname_crestr (process_table, OZ_PROCMODE_KNL, seckeys, secattr, 0, 8, "OZ_DEBUG", "1", &logname_debug);
    if (sts != OZ_SUCCESS) goto rtn;
  }
...
rtn:
  ...
  if (logname_debug     != NULL) oz_knl_logname_increfc (logname_debug,     -1);
    ...
    if (sts != OZ_SUCCESS) oz_sys_io_fs_printerror ("oz_sys_spawn: error %u looking up logname OZ_IMAGE\n", sts);
      ...
      if (sts != OZ_SUCCESS) oz_sys_io_fs_printerror ("oz_sys_spawn: error %u getting imagename from OZ_IMAGE\n", sts);
        ...
        if (sts != OZ_SUCCESS) oz_sys_io_fs_printerror ("oz_sys_spawn: error %u loading image %s\n", sts, imagename);
          ...
          oz_sys_io_fs_printerror ("oz_sys_spawn: null start address image %s\n", imagename);
          sts = OZ_BADSTARTADDR;
        }
