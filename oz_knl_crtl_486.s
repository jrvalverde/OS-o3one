...
stpncpy:
	...
	movl	 8(%ebp),%edi	# get output buffer address
	jecxz	stpncpy_done	# all done if output buffer size zero
	movl	12(%ebp),%esi	# get input buffer address
	cld			# copy forward
stpncpy_loop:
	...
	loopnz	stpncpy_loop	# decrement %ecx, then loop if %ecx and %al are both nonzero
	jecxz	stpncpy_done	# done if output buffer filled up
	rep			# now pad the output buffer with nulls
	stosb
stpncpy_done:
	movl	%edi,%eax	# return end of output
	popl	%esi		# restore registers
	popl	%edi
	popl	%ebp
	ret
...
divdi3_divisorneg:
	...
	popl	%ebp			# ... then use result as is
	ret
