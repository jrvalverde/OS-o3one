...
uLong int_tcp_send (const char *name, void *dummy, int argc, char *argv[]);
...
static const Command intcmd[] = {
	...
	"tcp send",        int_tcp_send,    NULL, "<hostname> <portno> <file>", 
	NULL, NULL, NULL, NULL };
...

{
  ...
  uByte buffer[65536], ipaddr[OZ_IO_IP_ADDRSIZE], portno[OZ_IO_IP_PORTSIZE];
  uLong sts;

  if (argc != 3) {
    oz_sys_io_fs_printf (oz_util_h_error, "%s: tcp send <hostname> <portno> <file>\n", pn);
    return (OZ_MISSINGPARAM);
  }

  /* Convert hostname and port number to binary */
  ...
  if (sts != OZ_SUCCESS) {
    oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u opening %s\n", pn, sts, fs_open.name);
    return (sts);
  }
  ...
  ip_tcpconnect.dstipaddr = ipaddr;
  ip_tcpconnect.dstportno = portno;
  ip_tcpconnect.timeout   = 30000;
  sts = oz_sys_io (OZ_PROCMODE_KNL, h_ipchan, 0, OZ_IO_IP_TCPCONNECT, sizeof ip_tcpconnect, &ip_tcpconnect);
  if (sts != OZ_SUCCESS) {
    oz_sys_handle_release (OZ_PROCMODE_KNL, h_file);
    oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u connecting to server\n", pn, sts);
    return (sts);
  }
  ...
  fs_readrec.size = sizeof buffer;
  fs_readrec.buff = buffer;
  fs_readrec.rlen = &ip_tcptransmit.rawsize;

  ip_tcptransmit.rawbuff = buffer;

  while (1) {
    sts = oz_sys_io (OZ_PROCMODE_KNL, h_file, 0, OZ_IO_FS_READREC, sizeof fs_readrec, &fs_readrec);
    if ((sts != OZ_SUCCESS) && (ip_tcptransmit.rawsize == 0)) break;
    sts = oz_sys_io (OZ_PROCMODE_KNL, h_ipchan, 0, OZ_IO_IP_TCPTRANSMIT, sizeof ip_tcptransmit, &ip_tcptransmit);
    if (sts != OZ_SUCCESS) {
      oz_sys_handle_release (OZ_PROCMODE_KNL, h_file);
      oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u transmitting to server\n", pn, sts);
      return (sts);
    }
  }
  ...
  oz_sys_handle_release (OZ_PROCMODE_KNL, h_file);
  if (sts != OZ_ENDOFFILE) {
    oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u reading file\n", pn, sts);
    oz_sys_io (OZ_PROCMODE_KNL, h_ipchan, 0, OZ_IO_IP_TCPCLOSE, 0, NULL);
    return (sts);
  }

  /* Close connection */

  sts = oz_sys_io (OZ_PROCMODE_KNL, h_ipchan, 0, OZ_IO_IP_TCPCLOSE, 0, NULL);
  if (sts != OZ_SUCCESS) oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u closing connection\n", pn, sts);
  return (sts);
}

/************************************************************************/
