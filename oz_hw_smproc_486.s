...
##########################################################################
##									##
##  Local APIC timer routines						##
##									##
##  Each cpu is responsible for its own quantum timer.			##
##									##
##########################################################################
...
oz_hw_quantimer_start:
	####		ret	####		# uncommenting safely inhibits quantum timer

	pushl	%ebp
	movl	%esp,%ebp
	movzwl	14(%ebp),%eax			# get high-order delta iota expiration time
	xorl	%edx,%edx
	divl	rdtsc_per_apic_16		# convert to delta timer count
	testl	%eax,%eax
	movl	10(%ebp),%eax			# get mid-order delta iota expiration time
	jne	quantimer_start_over
	divl	rdtsc_per_apic_16		# convert to delta timer count
	cmpl	$128,%eax
	movl	%eax,%ecx
	jnc	quantimer_start_over		# - we can only handle a 38 bit result
	movl	 2(%ebp),%eax			# get low-order delta iota expiration time
	xorw	%ax,%ax
	divl	rdtsc_per_apic_16		# convert to delta timer count
	movl	$0x0B,%edx			# init the divisor to 1 (ie, clock speed = bus speed / 1)
	shrdl	$16,%ecx,%eax			# shift over by 16
	shrl	$16,%ecx
	jne	quantimer_start_scale
quantimer_start_setup:
	movl	$0x10000,APIC_L_LCLVTBL		# mask off the timer interrupt
	movl	%edx,APIC_L_TIMDVCR		# set the scale factor
	movl	%eax,APIC_L_TIMINIT		# set up count & start timing
	movl	$INT_QUANTIM,APIC_L_LCLVTBL	# enable timer interrupt
	popl	%ebp
	ret
quantimer_start_scale:
	incl	%edx				# increment divisor
	shrdl	$1,%ecx,%eax			# shift count
	shrl	$1,%ecx
	jne	quantimer_start_scale
	addl	$4,%edx				# fix divisor
	movl	%edx,%ecx
	andl	$4,%ecx
	addl	%ecx,%edx
	andl	$15,%edx
	jmp	quantimer_start_setup
quantimer_start_over:
	movl	$0x0A,%edx			# overflow, set up max time
	movl	$-1,%eax			# = 2^32 * 128 (hours?)
	jmp	quantimer_start_setup

##########################################################################
##									##
...
getcpu_chkhwi:
	...
	pushl	$cpu_getcur_msg1
	call	oz_crash
getcpu_msg1: .string "oz_hw486_getcpu: called from %p with APIC_L_PROCPRI of %X, eflags %X"
...
# Initialize time-of-day and interval timer

	call	oz_hw486_tod_init
	call	oz_hw486_timer_init

## Now wait a second to count the bus (apic) speed, or just wait an eighth then multiply by eight
## The local apic timer is used for quantum timing in an smp environment
	...
	movl	%eax,apic_second		# save how many counts it gets in a second
	pushl	%eax
	pushl	$apic_msg3
	call	oz_knl_printk
	addl	$12,%esp
...
oz_hw486_irq_many_add:
	...
	testb	$0xFD,%bl				# check for IRQ 0 or 2
	jne	irq_many_add_lock
	xorb	$0x02,%bl				# swap them - IRQ 0 for timer comes in on IRQ 2 line of IOAPIC
irq_many_add_lock:
