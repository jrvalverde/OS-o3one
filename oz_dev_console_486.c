...
static int conport_486_shutdown (OZ_Devunit *devunit, void *devexv);
...
static OZ_Devfunc conport_486_functable = { sizeof (Devex), sizeof (Chnex), sizeof (Iopex), 0, conport_486_shutdown, NULL, NULL, conport_486_assign, 
...
static void enabledisable (uByte code);
static uLong enabdiswait (void *dummy);

/************************************************************************/
/*									*/
...

{
  ...
  if (!initialized) {
    ...
    enabledisable (0xF4);					/* enable keyboard */
  ...
  while ((bl = oz_dev_keyboard_getc (0)) != 0) {
    ...
    if (bl > 0) {
      if (cdevex != NULL) {
        ...
        if (oz_dev_video_mode_local_echo) oz_dev_vgavideo_putchar (cdevex -> vctx, bl);
        if ((bl == 13) && oz_dev_video_mode_newline) {
          (*(cdevex -> comport_setup.class_kbd_char)) (cdevex -> comport_setup.class_param, 10);
          if (oz_dev_video_mode_local_echo) oz_dev_vgavideo_putchar (cdevex -> vctx, 10);
        }
      }
      continue;
    }
...
/*    Output:								*/
/*									*/
/*	oz_dev_atkcons_getcon = 0 : end-of-file terminator		*/
/*	                        1 : normal terminator			*/
...
int oz_dev_atkcons_getcon (uLong size, char *buff, uLong pmtsize, const char *pmtbuff)

{
  char c;
  uLong kb, offs;

  if (!kbenabled) enabledisable (0xF4);
...
int oz_knl_atkcons_debugchk (void)
...
chkmb:
  ...
  /* - ignmb is set by the internal read routines like oz_dev_atkcons_getcon */
...

{
  ...
  if (sts != OZ_SUCCESS) {
    oz_knl_printk ("oz_dev_atkcons_init: error %u assigning channel to console class device 'conclass'\n", sts);
    return (NULL);
  }
  ...
  if (sts != OZ_SUCCESS) oz_crash ("oz_dev_atkcons_init: error %u setting up console device %d\n", sts, vidx);
...
/************************************************************************/
/*									*/
/*  Send enable / disable command to keyboard				*/
/*									*/
/************************************************************************/
...
{
  oz_hw486_outb (0xAD, KB_CP);					// disable keyboard interrupts
  oz_hw486_outb (code, KB_DP);					// output enable (F4) or disable (F5) code
  if (oz_hw_stl_microwait (100000, enabdiswait, NULL) == 0) {	// wait a little bit for an response (FA)
    oz_knl_printk ("oz_dev_console: timed out waiting for keyboard %s\n", (code == 0xF5) ? "disable" : "enable");
  }
  kbenabled = (code == 0xF4);
  if (kbenabled) oz_hw486_outb (0xAE, KB_CP);			// enable keyboard interrupts
}

static uLong enabdiswait (void *dummy)

{
  return ((oz_hw486_inb (KB_CP) & 0x01) && (oz_hw486_inb (KB_DP) == 0xFA));
}
