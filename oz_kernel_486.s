...
oz_hw486_kernel_idt:	.space	256*16			# *16 correct, see oz_hw486_idtinit
...
#        they must check for usermode ast's before iretting to user mode
			...
			#                    (every EXECODESEG handler must break stack frame chain if prev mode not executive)
...
kernel_idt_fix:	.long	 0,0x01,CONCODESEG,exception_DE				# divide error
		.long	 1,0x00,EXECODESEG,exception_DB				# debug - call condition handler
...
.endif
		.long	INT_RDEBUG,0x60,KNLCODESEG,except_rdebug		# read debug registers
		.long	INT_WDEBUG,0x60,KNLCODESEG,except_wdebug		# write debug registers
		.long	-1
...
## is print the registers, a traceback, then hang.
	...
	call	oz_hw486_idtinit		# fill it in
...
.if 0000
	pushl	$1				# make sure some of the new kernel stack is writable
...
#	        %ax = usermode %cs (any non-zero value will work)
#	%eax<31:16> = undefined
...
syscall_sep_rtnsts:
	...
	call	oz_knl_thread_chkpendast	# ... see if there will be any deliverable ast's for that mode
	addl	$4,%esp
	movl	$syscall_sep_rtnnoasts,%edx	# assume no deliverable ast's
	testl	%eax,%eax
	...
	popl	%ecx				# get T-bit flag
	popl	%ebx				# restore saved %ebx
	jecxz	syscall_sep_tbitset
...
syscall_sep_bad:
	...
	jmp	syscall_sep_rtnsts

	.align	4
syscall_sep_rtnnoasts:
	popl	%ebp				# back in user mode, pop saved %ebp
	ret					# status is in %eax

# T-bit is to be set on return

syscall_sep_tbitset:
	popl	%ebp				# get user frame pointer = user stack pointer
	pushl	$USRDATASEG			# push user stack segment number
	pushl	%ebp				# push user stack pointer value
	pushfl					# push eflags
	orb	$3,1(%esp)			# ... with IF and TF set
	pushl	$USRCODESEG			# push user code segment number
	pushl	%edx				# push return address
	iret					# return and enable interrupts and tracing at the same time

##########################################################################
##									##
##  Exception handlers							##
...
exception_BP:				# 3 - int3 breakpoint
	...
	movl	4(%ebp),%eax		#     get eip just after the int3
	cmpb	$0xCC,-1(%eax)		#     see if it is an int3
	jne	1000f			#     bozo did an 'int $3'
	decl	4(%ebp)			#     'int3', decrement to int3 instruction
1000:
	popl	%eax			#     restore scratch register
	movl	oz_BREAKPOINT,%ebp	#     go signal breakpoint exception
	jmp	exception_xx
...
exception_AC:				# 17 - alignment check
	xchg	(%esp),%ebp		# just throw away the always-zero error code
...
exception_MF_push:
	...
	.align	4			####
exception_MF_nofpu:			####
	movl	%esp,%eax		#### fp exception with fpu disabled
...
##  Debug exception							##
##									##
##  This is called in kernel mode with interrupt delivery inhibited	##
##									##
...
exception_DB:				# 1 - debug exception
	pushl	%ebp
	movl	%dr6,%ebp		# set ec1=dr6
	pushal				# build mchargs on stack
	xorl	%eax,%eax		# set dr6=0
	movl	%eax,%dr6
	pushl	%eax			# and push dummy ec2 error code
	...
	testb	$2,MCH_L_EFLAGS+1(%esp)	# see if caller had interrupts enabled
	je	1000f
	sti				# if so, it's ok to enable them now that we have read dr6
1000:
	xorl	%eax,%eax		# no sigargs pushed on stack yet
	movl	MCH_L_EC1(%esi),%ecx	# get dr6 contents
	bt	$14,%ecx		# see if singlestep caused exception
	jnc	2000f
	cmpl	$syscall_sep_enter,MCH_L_EIP(%esp)
	je	8000f
...
5000:
	...
	addl	$3,%eax
6000:
	pushl	%eax			# push number of longs in sigargs (not incl this long)
	incl	%eax			# number of longs on stack until mchargs
	call	oz_hw486_movemchargstocallerstack # if caller was user mode, move schmeel to user stack
	cmpl	oz_SUCCESS,%eax		# ... pagefaulting as necessary, then switch to user mode
	je	exception_test		# if successful, go figure out what to do with it all
	movl	%eax,%ebx		# save failure code (probably oz_ACCVIO)
	pushl	MCH_L_ESP(%esi)		# output message to console indicating why we are aborting it
	pushl	%ebx
	pushl	$except_db_msg1
	call	oz_knl_printk
	pushl	%ebx			# exit thread with failure code as status
7000:
	call	oz_knl_thread_exit
	jmp	7000b

	# We are here because the T-bit was set on entry to the SYSENTER routine
	# So we clear the return EAX as well as the return T-bit
	# The syscall_sep_enter routine will turn the T-bit back on when it returns
	# Interrupt delivery should still be inhibited

8000:
	movl	$0,MCH_L_EAX(%esp)	# clear %eax (it used to have user's %cs in it)
	andb	$0xFE,MCH_L_EFLAGS+1(%esp) # clear T-bit
	jmp	exception_rtn		# we're done

except_db_msg1:	.string	"oz_kernel_486 exception_DB: error %u locking user stack (below %p) in memory\n"

##########################################################################
##									##
##  These exception handlers are called in the mode of the instruction 	##
...
oz_hw_mchargx_fetch:
	...
	movw	%gs,%dx
	movw	%ss,%di
	movw	%ax,MCHX_W_DS(%esi)
	movw	%bx,MCHX_W_ES(%esi)
	movw	%cx,MCHX_W_FS(%esi)
	movw	%cs,%cx
	movw	%dx,MCHX_W_GS(%esi)
	testb	$2,%cl
	movw	%di,MCHX_W_SS(%esi)
	jne	1000f
	MOVL_CR0_EAX
	movl	%eax,MCHX_L_CR0(%esi)
	MOVL_CR2_EAX
	movl	%eax,MCHX_L_CR2(%esi)
	MOVL_CR3_EAX
	movl	%eax,MCHX_L_CR3(%esi)
	MOVL_CR4_EAX
	movl	%eax,MCHX_L_CR4(%esi)
	jmp	2000f
1000:
	xorl	%eax,%eax
	movl	%eax,MCHX_L_CR0(%esi)
...
2000:
	int	$INT_RDEBUG
	movl	%eax,MCHX_L_DR0(%esi)
	movl	%ebx,MCHX_L_DR1(%esi)
	movl	%ecx,MCHX_L_DR2(%esi)
	movl	%edx,MCHX_L_DR3(%esi)
	movl	%edi,MCHX_L_DR7(%esi)
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
...
oz_hw_mchargx_store:
	...
	pushl	%esi
	pushl	%ebx
	movl	8(%ebp),%esi			# point to data struct
	movw	MCHX_W_DS(%esi),%ax		# write segment registers
	movw	MCHX_W_ES(%esi),%bx
	movw	MCHX_W_FS(%esi),%cx
	movw	MCHX_W_GS(%esi),%dx
	movw	MCHX_W_SS(%esi),%di
	movw	%ax,%ds
	movw	%bx,%es
	movw	%cx,%fs
	movw	%cs,%cx
	...
	movw	%di,%ss
	jne	1000f
	movl	MCHX_L_CR0(%esi),%eax		# write control registers
	MOVL_EAX_CR0
.if !PARANOID_MPD
	movl	MCHX_L_CR3(%esi),%eax
	MOVL_EAX_CR3
.endif
	movl	MCHX_L_CR4(%esi),%eax
	MOVL_EAX_CR4
1000:
	movl	12(%ebp),%edx			# point to mask struct
	movl	MCHX_L_DR0(%edx),%eax		# see if any debug registers being written
	orl	MCHX_L_DR1(%edx),%eax
	orl	MCHX_L_DR2(%edx),%eax
	orl	MCHX_L_DR3(%edx),%eax
	orl	MCHX_L_DR7(%edx),%eax
	je	2000f				# if not, don't activate them on this thread
	movl	MCHX_L_DR0(%esi),%eax		# if so, write them all, activating if necessary
	movl	MCHX_L_DR1(%esi),%ebx
	movl	MCHX_L_DR2(%esi),%ecx
	movl	MCHX_L_DR3(%esi),%edx
	movl	MCHX_L_DR7(%esi),%edi
	int	$INT_WDEBUG
2000:
	popl	%ebx
	popl	%esi
...
# Read and Write Debug Registers
...
except_wdebug:
	...
	movl	%ecx,%dr2
	movl	%edx,%dr3
	movl	%dr7,%eax		# disallow changing reserved bits and the GD bit
	andw	$0xFC00,%ax
	andw	$0x03FF,%di
	orl	%eax,%edi
	movl	%edi,%dr7
	call	oz_hw486_getcpu		# point to cpu database
	movl	CPU_L_THCTX(%esi),%eax	# point to current thread context block
	movb	$1,THCTX_B_DBUSED(%eax)	# this thread is using debug registers
	iret
