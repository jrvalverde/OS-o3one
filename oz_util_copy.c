...

{
  ...
  if (oblocksize > sizeof buff) goto record_by_record;
  ...
  if (fs_getinfo1.blocksize == 0) goto record_by_record;
  if (fs_getinfo1.blocksize > sizeof buff) goto record_by_record;

  if ((oblocksize < fs_getinfo1.blocksize) && ((fs_getinfo1.blocksize % oblocksize) != 0)) goto record_by_record;
  if ((fs_getinfo1.blocksize < oblocksize) && ((oblocksize % fs_getinfo1.blocksize) != 0)) goto record_by_record;

  /* Disk-to-disk copy, extend output file to size of input file */

  inputbytes = fs_getinfo1.eofblock;
  if (inputbytes != 0) {
    inputbytes --;
    inputbytes *= fs_getinfo1.blocksize;
    inputbytes += fs_getinfo1.eofbyte;
    if (inputbytes != 0) {
      memset (&fs_extend, 0, sizeof fs_extend);
      fs_extend.nblocks = (inputbytes + oblocksize - 1) / oblocksize;
  ...
  fs_readblocks.size  = sizeof buff;
  fs_readblocks.buff  = buff;
  fs_readblocks.svbn  = 1;

  fs_writeblocks.size = sizeof buff;
  fs_writeblocks.buff = buff;
  fs_writeblocks.svbn = 1;
  ...
  bytestogo = inputbytes;
  while (bytestogo > sizeof buff) {
    ...
    bytestogo -= sizeof buff;
    fs_readblocks.svbn  += fs_readblocks.size  / fs_getinfo1.blocksize;
    fs_writeblocks.svbn += fs_writeblocks.size / oblocksize;
    count += sizeof buff / oblocksize;
  }

  /* Copy the last partial bit */

  fs_readblocks.size = (bytestogo + fs_getinfo1.blocksize - 1) / fs_getinfo1.blocksize * fs_getinfo1.blocksize;
  sts = oz_sys_io (OZ_PROCMODE_KNL, h_in, 0, OZ_IO_FS_READBLOCKS, sizeof fs_readblocks, &fs_readblocks);
  if (sts != OZ_SUCCESS) {
    oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u reading input file %s at %u\n", pn, sts, iname, fs_readblocks.svbn);
    goto rtn;
  ...
  fs_writeblocks.size = (bytestogo + oblocksize - 1) / oblocksize * oblocksize;
  sts = oz_sys_io (OZ_PROCMODE_KNL, h_out, 0, OZ_IO_FS_WRITEBLOCKS, sizeof fs_writeblocks, &fs_writeblocks);
  if (sts != OZ_SUCCESS) {
    oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u writing output file %s at %u\n", pn, sts, oname, fs_writeblocks.svbn);
    goto rtn;
  ...
  fs_getinfo1.eofblock = (inputbytes / oblocksize) + 1;
  fs_getinfo1.eofbyte  = inputbytes % oblocksize;
