...
#define DFS_HEADER_AREA_FILNAME (0)
#define DFS_HEADER_AREA_FILATTR (1)
#define DFS_HEADER_AREA_SECATTR (2)
#define DFS_HEADER_AREA_POINTER (3)
#define DFS_HEADER_NAREAS (4)

/* Point to the pointers in a file block */

#define POINTERS(__filex) (Pointer *)(__filex -> header.area + __filex -> header.areas[DFS_HEADER_AREA_POINTER].offs)

/* Point to filename string in an header */

#define FILENAME(__header) (char *)((__header).area + (__header).areas[DFS_HEADER_AREA_FILNAME].offs)

/* Get the end-of-file block and byte numbers from an header */

#define FILATTRS(__header) ((Filattr *)((__header).area + (__header).areas[DFS_HEADER_AREA_FILATTR].offs))
...
typedef struct { uWord headerver;			/* header version */
                 ...
                 struct { uWord size, offs; } areas[DFS_HEADER_NAREAS];
                 uByte area[1];
               } Header;
...
static char const *dfs_get_volname (OZ_VDFS_Volume *volume);
...
static uLong dfs_init_volume (OZ_VDFS_Devex *devex, OZ_VDFS_Volume *volume, int volnamelen, char const *volname, uLong clusterfactor, uLong secattrsize, const void *secattrbuff, uLong initflags, OZ_VDFS_Iopex *iopex);
...
static uLong dfs_lookup_file (OZ_VDFS_File *dirfile, int namelen, char const *name, OZ_VDFS_Fileid *fileid_r, char *name_r, OZ_VDFS_Iopex *iopex);
static uLong dfs_rename_file (OZ_VDFS_File *olddirfile, char const *oldname, OZ_VDFS_File *newdirfile, char const *newdirname, int newnamelen, char const *newname, int newversion, OZ_VDFS_File *file, const OZ_VDFS_Fileid *fileid, char *newname_r, OZ_VDFS_Iopex *iopex);
static uLong dfs_enter_file (OZ_VDFS_File *dirfile, char const *dirname, int namelen, char const *name, int newversion, OZ_VDFS_File *file, const OZ_VDFS_Fileid *fileid, char *name_r, OZ_VDFS_Iopex *iopex);
static uLong dfs_remove_file (OZ_VDFS_File *dirfile, char const *name, char *name_r, OZ_VDFS_Iopex *iopex);
static void dfs_returnspec (char *spec, uLong size, char *buff, OZ_FS_Subs *subs);
static uLong dfs_create_file (OZ_VDFS_Volume *volume, int namelen, char const *name, uLong filattrflags, OZ_VDFS_Fileid *dirid, OZ_VDFS_File *file, OZ_VDFS_Fileid **fileid_r, OZ_VDFS_Iopex *iopex);
...
static uLong lookup_file (OZ_VDFS_File *dirfile, int namelen, char const *name, OZ_Dbn *dirvbn_r, OZ_VDFS_Fileid *fileid_r, char *name_r, OZ_VDFS_Iopex *iopex);
...

{
  ...
  movc4 (filex -> header.areas[DFS_HEADER_AREA_FILATTR].size, filex -> header.area + filex -> header.areas[DFS_HEADER_AREA_FILATTR].offs, sizeof filattr, &filattr);
...
static int dfs_atofid (char const *buff, void *fileid);
...

{
  ...
  areaoffs = 0;
  for (i = 0; i < DFS_HEADER_NAREAS; i ++) {
    header -> areas[i].offs = areaoffs;
    switch (i) {

      /* Fill in filename including the null */

      case DFS_HEADER_AREA_FILNAME: {
        header -> areas[DFS_HEADER_AREA_FILNAME].size = strlen (name) + 1;
        memcpy (header -> area + areaoffs, name, header -> areas[DFS_HEADER_AREA_FILNAME].size);
        break;
      }

      /* Fill in file attributes */

      case DFS_HEADER_AREA_FILATTR: {
        ...
        header -> areas[DFS_HEADER_AREA_FILATTR].size = sizeof filattr;
      ...
      case DFS_HEADER_AREA_SECATTR: {
        header -> areas[DFS_HEADER_AREA_SECATTR].size = secattrsize;
      ...
      case DFS_HEADER_AREA_POINTER: {
        if (count != 0) {
          header -> areas[DFS_HEADER_AREA_POINTER].size = sizeof (Pointer);
...
/*    Input:								*/
/*									*/
/*	olddirfile = old directory file					*/
/*	oldname    = old file name					*/
/*									*/
/*	newdirfile = new directory file					*/
/*	newdirname = new directory name (diag only)			*/
/*	newnamelen = length of new name to enter			*/
/*	newname    = new name to enter					*/
...
/*    Output:								*/
/*									*/
/*	rename_file = OZ_SUCCESS : successful				*/
/*	                    else : error status				*/
/*	*newname_r  = filled in with resultant name (incl version)	*/
/*									*/
/************************************************************************/
...

{
  ...
  entfile = file;
  if (entfile == NULL) {
    sts = oz_dev_vdfs_open_file (olddirfile -> volume, fileid, 0, &entfile, iopex);
    if (sts != OZ_SUCCESS) return (sts);
  }

  /* Enter the file in the new directory */

  sts = dfs_enter_file (newdirfile, newdirname, newnamelen, newname, newversion, entfile, fileid, newname_r, iopex);

  /* Remove it from the old directory */

  if (sts == OZ_SUCCESS) sts = dfs_remove_file (olddirfile, oldname, NULL, iopex);

  /* Write new directory and name to the file's header.  Do this regardless of whether the  */
  /* header has the old name to allow someone to 'fix' a broken backlink by doing a rename. */

  if (sts == OZ_SUCCESS) {
    filex = entfile -> filex;
    filex -> header.areas[DFS_HEADER_AREA_FILNAME].size = 0;								// wipe out old name
    sts = make_header_room (entfile, filex, newnamelen + 1, DFS_HEADER_AREA_FILNAME, 1, &extfilex, iopex);		// make room for new name
    if (sts == OZ_SUCCESS) {
      filex -> header.dirid = newdirfile -> filex -> header.fileid;							// set up new directory id
      extfilex -> header.areas[DFS_HEADER_AREA_FILNAME].size = newnamelen + 1;						// set new name length incl null
      memcpy (extfilex -> header.area + extfilex -> header.areas[DFS_HEADER_AREA_FILNAME].offs, newname, newnamelen);	// copy name string
      extfilex -> header.area[extfilex->header.areas[DFS_HEADER_AREA_FILNAME].offs+newnamelen] = 0;			// put in a null
      oz_dev_vdfs_mark_header_dirty (entfile);										// header needs to be written back out
      mark_exthdr_dirty (extfilex, entfile);
    }
  }

  if (entfile != file) oz_dev_vdfs_close_file (entfile, iopex);
...

{
  ...
  *fileid_r = &(filex -> header.fileid);
  ...
  sts = make_header_room (file, filex, namelen + 1, DFS_HEADER_AREA_FILNAME, 0, &extfilex, iopex);
  if (sts != OZ_SUCCESS) goto rtnerr;

  extfilex -> header.areas[DFS_HEADER_AREA_FILNAME].size = namelen + 1;
  i = extfilex -> header.areas[DFS_HEADER_AREA_FILNAME].offs;
  ...
  sts = make_header_room (file, filex, sizeof filattr, DFS_HEADER_AREA_FILATTR, 0, &extfilex, iopex);
  if (sts != OZ_SUCCESS) goto rtnerr;
  ...
  extfilex -> header.areas[DFS_HEADER_AREA_FILATTR].size = sizeof filattr;
  *(Filattr *)(extfilex -> header.area + extfilex -> header.areas[DFS_HEADER_AREA_FILATTR].offs) = filattr;
  mark_exthdr_dirty (extfilex, file);

  /* Copy security attributes into header */

  secattrsize = oz_knl_secattr_getsize (file -> secattr);
  sts = make_header_room (file, filex, secattrsize, DFS_HEADER_AREA_SECATTR, 1, &extfilex, iopex);
  if (sts != OZ_SUCCESS) goto rtnerr;

  extfilex -> header.areas[DFS_HEADER_AREA_SECATTR].size = secattrsize;
  memcpy (extfilex -> header.area + extfilex -> header.areas[DFS_HEADER_AREA_SECATTR].offs, oz_knl_secattr_getbuff (file -> secattr), secattrsize);
...

{
  ...
  secsize = filex -> header.areas[DFS_HEADER_AREA_SECATTR].size;
  while (filex -> header.extid.num != 0) {
    ...
    secsize += extfilex -> header.areas[DFS_HEADER_AREA_SECATTR].size;
    filex = extfilex;
  }
  filex = file -> filex;
  ...
  if (secsize != 0) {
    ...
    for (extfilex = filex; extfilex != NULL; extfilex = extfilex -> next) {
      memcpy (secbuff + i, extfilex -> header.area + extfilex -> header.areas[DFS_HEADER_AREA_SECATTR].offs, extfilex -> header.areas[DFS_HEADER_AREA_SECATTR].size);
      i += extfilex -> header.areas[DFS_HEADER_AREA_SECATTR].size;
    }
...

{
  ...
  while (relblock != 0) {
    ...
    startlbn = 0;
    if (extfilex2 -> header.areas[DFS_HEADER_AREA_POINTER].size > 0) {
      pointer += extfilex2 -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer;
      startlbn = pointer[-1].logblock + pointer[-1].blockcount;
    }

    /* Make room in the header for the new pointer */

    sts = make_header_room (file, extfilex2, sizeof (Pointer), DFS_HEADER_AREA_POINTER, !(extflags & EXTEND_NOEXTHDR), &extfilex2, iopex);
    if (sts != OZ_SUCCESS) return (sts);
    ...
    pointer = POINTERS (extfilex2) + extfilex2 -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer; /* point to where to put new pointer */

    if ((extfilex2 -> header.areas[DFS_HEADER_AREA_POINTER].size >= sizeof *pointer) 				/* if there is at least one pointer there ... */
     && (pointer[-1].blockcount + pointer[-1].logblock == logblock)) {						/* ... and it ends where new one starts ... */
      ...
      extfilex2 -> header.areas[DFS_HEADER_AREA_POINTER].size += sizeof *pointer;				/* increase pointer area size */
    }
...
truncate_it:
  ...
  for (; i < extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof (Pointer); i ++) {
  ...
  for (extfilex = file -> filex; extfilex != NULL; extfilex = extfilex -> next) {
    ...
    pointer = POINTERS (extfilex);
    for (i = 0; i < extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof (Pointer); i ++) {
...
trunc_found_ptr:
  ...
  extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size = (++ i) * sizeof (Pointer);
...
trunc_found_hdr:
  ...
  extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size = savei * sizeof (Pointer);

  /* Release all the blocks for all extension headers following this one */

  while ((extfilex2 = extfilex -> next) != NULL) {
    pointer = POINTERS (extfilex2);
    for (i = extfilex2 -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof (Pointer); i > 0; -- i) {
      nfound = pointer -> blockcount;
      file -> allocblocks      -= nfound;
      extfilex2 -> blocksinhdr -= nfound;
      free_blocks (volume, nfound, pointer -> logblock, iopex);
    ...
    extfilex2 -> header.areas[DFS_HEADER_AREA_POINTER].size = 0;
...
insert_it:
  ...
  numpoints = extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer;
  pointer   = POINTERS (extfilex);
  logblock  = 0;
  ...
  if (relblock != 0) {
    ...
    if (sts != OZ_SUCCESS) {
      ...
      extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size -= sizeof *pointer;
      memmove (pointer, pointer + 1, extfilex -> header.area 
                                   + extfilex -> header.areas[DFS_HEADER_AREA_POINTER].offs 
                                   + extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size 
                                   - (uByte *)pointer);
      goto rtnerr;
    }
...
remove_it:
  numpoints = extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer;	// get number of pointers in the header
  ...
  while (nblocks != 0) {
    ...
    if (relblock == 0) {
      ...
      extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size -= sizeof *pointer;	// reduce number of pointers in header
      mark_exthdr_dirty (extfilex, file);						// this header is dirty
...

{
  ...
  if (pointidx > extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer) {
    oz_crash ("oz_dev_dfs makeroomforapointer: bad pointer %p for filex %p", pointer, extfilex);
  }
  numpoints = extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer - pointidx;

  /* Hopefully there is room in the current header for the new pointer */

  sts = make_header_room (file, extfilex, sizeof *pointer, DFS_HEADER_AREA_POINTER, 0, &extfilex2, iopex);
  if (sts == OZ_SUCCESS) {
    if (extfilex2 != extfilex) oz_crash ("oz_dev_dfs makeroomforapointer: extfilex %p, extfilex2 %p", extfilex, extfilex2);
    extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size += sizeof *pointer;	// header has another pointer now
  ...
  if (extfilex2 != NULL) {
    sts = make_header_room (file, extfilex2, sizeof *pointer, DFS_HEADER_AREA_POINTER, 0, &extfilex3, iopex);
    if (sts == OZ_SUCCESS) {
  ...
  sts = make_header_room (file, extfilex, sizeof *pointer, DFS_HEADER_AREA_POINTER, 1, &extfilex2, iopex);
  if (sts != OZ_SUCCESS) return (sts);
...
use_ext2:
  ...
  memmove (pointer2 + 1, pointer2, extfilex2 -> header.areas[DFS_HEADER_AREA_POINTER].size);	// shift things down to make room
  extfilex2 -> header.areas[DFS_HEADER_AREA_POINTER].size += sizeof *pointer;			// new header now has another pointer
...

{
  ...
  for (filex = file -> filex; filex != NULL; filex = filex -> next) {
    ...
    for (i = filex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer; i > 0; -- i) {
  ...
  if (nblockstotal != 0) {
    ...
    for (filex = file -> filex; filex != NULL; filex = filex -> next) {
      pointer = POINTERS (filex);
      for (i = 0; i < filex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer; i ++) {
...

{
  ...
  *filex_r = filex;
  ...
  for (i = DFS_HEADER_NAREAS; -- i > narea;) {
...

{
  ...
  usedsize = 0;
  for (i = 0; i < DFS_HEADER_NAREAS; i ++) usedsize += (filex -> header.areas[i].size + 3) & -4;
  return (usedsize);
}

/************************************************************************/
...

{
  ...
  if (filex -> header.areas[DFS_HEADER_AREA_POINTER].size != 0) oz_crash ("oz_dev_dfs delete_header: called with blocks allocated");
...
found_extension:
  ...
  pointer = POINTERS (extfilex);
  n = extfilex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer;
  for (i = 0; i < n; i ++) {
...

{
  ...
  blocksinhdr = 0;
  npointers   = filex -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointers;
...

{
  ...
  if (header -> areas[DFS_HEADER_AREA_FILNAME].size != 0) {
    p = (char *)(header -> area + header -> areas[DFS_HEADER_AREA_FILNAME].offs);
    if (strlen (p) >= header -> areas[DFS_HEADER_AREA_FILNAME].size) {
...
#if 00
  if (header -> areas[DFS_HEADER_AREA_FILATTR].size != 0) {
    movc4 (header -> areas[DFS_HEADER_AREA_FILATTR].size, header -> area + header -> areas[DFS_HEADER_AREA_FILATTR].offs, sizeof filattr, &filattr);
    ??
  }
#endif

#if 00
  if (header -> areas[DFS_HEADER_AREA_SECATTR].size != 0) ??
#endif

  if (header -> areas[DFS_HEADER_AREA_POINTER].size != 0) {
    if (header -> areas[DFS_HEADER_AREA_POINTER].size % sizeof *pointer != 0) {
      oz_dev_vdfs_printk (iopex, "oz_dev_dfs validate_header: pointer area size %u not multiple of pointer size\n", header -> areas[DFS_HEADER_AREA_POINTER].size);
      return (0);
    }
    pointer = (Pointer *)(header -> area + header -> areas[DFS_HEADER_AREA_POINTER].offs);
    for (i = 0; i < header -> areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointer; i ++) {
...
#define EOFBLOCK(file) ((Filattr *)(file -> header.area + file -> header.areas[DFS_HEADER_AREA_FILATTR].offs)) -> eofblock
...
scanheaders:
  ...
  for (filenumber = 1; filenumber <= numberoffiles; filenumber ++) {
    ...
    pointers = (Pointer *)(header -> area + header -> areas[DFS_HEADER_AREA_POINTER].offs);
    for (i = 0; i < header -> areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointers; i ++) {
    ...
    headersaves[filenumber-1].filattrflags = ((Filattr *)(header -> area + header -> areas[DFS_HEADER_AREA_FILATTR].offs)) -> filattrflags;
...

{
  ...
  while (vfy_rvb (vpb, indexheaders, filenumber, BLOCKSIZE, header) == OZ_SUCCESS) {
    ...
    filename = (char *)(header -> area + header -> areas[DFS_HEADER_AREA_FILNAME].offs);
    l = strlen (filename);
    if (l > i) break;
    i -= l;
...

{
  ...
  do {
    pointers = (Pointer *)(file -> header.area + file -> header.areas[DFS_HEADER_AREA_POINTER].offs);
    for (i = 0; i < file -> header.areas[DFS_HEADER_AREA_POINTER].size / sizeof *pointers; i ++) {
