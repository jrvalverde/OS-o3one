...
#define ETH_ZLEN (60)			/* smallest number of bytes to transmit */
					/* includes dstaddr,srcaddr,proto,userdata but excludes crc */
...
static void receive_finish (void *iopexv, uLong *status_r);
static void transmit_done (Devex *devex);
static void transmit_finish (void *iopexv, uLong *status_r);
...

{
  ...
  while (!(ether_inb (rtl -> iobase, IO_B_CR) & 1)) {			// see if a message is ready to process
    ...
    rx_size    = rx_status >> 16;					// split out the size (includes dst,src,proto,data,crc)

    if ((rx_status & (RxBadSymbol | RxRunt | RxTooLong | RxCRCErr | RxBadAlign)) ||
        (rx_size < ETH_ZLEN + 4) || (rx_size > BUFFSIZE + 4)) {		// check for nasty error
    ...
    for (i = 0; i < NUM_TX_DESC; i ++) {			/* loop through each transmit slot */
      ...
      if (iopex != NULL) {
        ...
        if (tsd & (TxStatOK | TxUnderrun | TxAborted | TxOutOfWindow | TxCarrierLost)) {	/* see if it is done */
          ...
          if (!(tsd & TxHostOwns)) {
            oz_knl_printk ("oz_dev_rtl8139 %s transmit_done: buffer %u done but not free (%X)\n", devex -> name, i, tsd);
...

{
  ...
  if ((*status_r != OZ_SUCCESS) || (iopex -> ether_transmit.xmtdrv_r == NULL)) OZ_KNL_NPPFREE (txbuf);
  else {
