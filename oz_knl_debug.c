...
static OZ_Mchargx *halted_mchargx[OZ_HW_MAXCPUS];
static uByte dc[OZ_DEBUG_SIZE];

static void crash_dump (OZ_Sigargs *sigargs, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx);

/************************************************************************/
/*									*/
/*  Callback routines							*/
...
static void kdb_abort (void *cp, OZ_Sigargs *sigargs, OZ_Mchargs *mchargs, OZ_Mchargx *mchargx)
...

{
  ...
  *symsize = 0;
  ...
  if (colon == NULL) {
    ...
    for (image = NULL; (image = oz_knl_image_next (image, 1, 0)) != NULL;) {		// scan list of images
...

{
  ...
  mchargx = halted_mchargx[oz_hw_cpu_getcur()];
  if (((OZ_Pointer)addr >= (OZ_Pointer)mchargx) 
   && (((OZ_Pointer)addr) + size <= (OZ_Pointer)(mchargx + 1))) {
    memset (((uByte *)addr) + sizeof mchargx[0], -1, size);
  }
...

{
  ...
  memset (halted_mchargx, 0, sizeof halted_mchargx);
...

{
  ...
  if (sigargs != NULL) switch (oz_s_loadparams.knl_exception) {
    case 1: {
      ...
      crash_dump (sigargs, mchargs, mchargx + 0);
  ...
  oz_hw_mchargx_fetch (mchargx + 0);
  halted_mchargx[cpuidx] = mchargx + 0;
  memset (mchargx + 1, 0, sizeof mchargx[1]);

  oz_sys_sprintf (sizeof prompt, prompt, "oz_knl_debug %d", oz_hw_cpu_getcur ());
  rc = oz_sys_debug_exception (prompt, (void *)dc, sigargs, mchargs, mchargx + 0);
  oz_hw_mchargx_store (mchargx + 0, mchargx + 1);
  halted_mchargx[cpuidx] = NULL;
  return (rc);
}
...

{
  ...
  halted_mchargs[cpuidx] = mchargs;						/* save mchargs/mchargx pointers for the cpu */
  halted_mchargx[cpuidx] = mchargx;
  if (oz_knl_crash_dump (cpuidx, sigargs, halted_mchargs, halted_mchargx)) {	/* do the crash dump */
...

{
  ...
  /* Save pointers to mchargs/mchargx if we're crash dumping */
  ...
  oz_sys_debug_halted (prompt, (void *)dc, mchargs, mchargx + 0);
  oz_hw_mchargx_store (mchargx + 0, mchargx + 1);
  halted_mchargx[cpuidx] = NULL;
}
