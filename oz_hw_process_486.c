...
typedef struct { OZ_Phyaddr ppdpa;	/* physical address of process private page directory page */
                 OZ_Section *ptsec;	/* process pagetable section */
                 OZ_Phyaddr pt1pa, pt2pa; /* physical address of the pagetable pages allocated by init routine */
...

{
  ...
  if (sysproc) {
    prctx -> ppdpa = MPDBASE;		/* use the global MPD at MPDBASE ... */
  ...
  4m boundary
	...
	|     this process' page tables		|  <- 0xFFFFE000  PROCMPDVADDR	<- physical address PRCTX_L_MPDPA
	+---------------------------------------+
	|  last RP page				|  <- 0xFFFFD000		<- physical address PRCTX_L_RP1PA
	|  last PT page				|  <- 0xFFFFC000		<- physical address PRCTX_L_PT1PA
	|  next to last PT page			|  <- 0xFFFFB000		<- physical address PRCTX_L_PT2PA
	|					|
	|  process page table pages		|				\  private section 'PRCTX_L_PTSEC'
	|  enough to map from PROCBASVADDR up 	|				|  - doesn't include PPD page
	|  to and including PROCMPDVADDR	|				|  - doesn't include last 2 PT pages
  ...
  */
  ...
  /* Create a section to put the pagetable and reqprot pages in (all except for the last three, */
  /* because they contain the pte and reqprot bytes that maps the process' page directory page) */

  sts = oz_knl_section_create (NULL, ONEMEG-PROCPTRPBVPAGE-5, 0, OZ_SECTION_TYPE_PAGTBL | OZ_SECTION_TYPE_ZEROES, NULL, &(prctx -> ptsec));
  if (sts != OZ_SUCCESS) {
    oz_knl_printk ("oz_hw_process_initctx: error %u creating pagetable section\n", sts);
    return (sts);
  }

  /* Make sure there is quota for four physical memory pages.  One is for the PPD, two are   */
  /* for the PT pages that map the PPD, one is for RP page that hold bits for last pt pages. */

  quota = oz_knl_section_getquota (prctx -> ptsec);
  if ((quota != NULL) && !oz_knl_quota_debit (quota, OZ_QUOTATYPE_PHM, 4)) {
    oz_knl_section_increfc (prctx -> ptsec, -1);
    return (OZ_EXQUOTAPHM);
  }

  /* Allocate a physical page for the PPD and initialize it with the global MPD contents         */
  ...
  prctx -> ppdpa = phypage << 12;						/* save corresponding physical address */
#if OZ_HW_PAGEPROT_NA != 0
  error : code assumes OZ HW PAGEPROT NA is zero
#endif
  oz_hw486_phys_filllong (0, prctx -> ppdpa);					/* fill with OZ_HW_PAGEPROT_NA entries for all pages */
  oz_hw_phys_movefromvirt (PROCBASVADDR >> 20, (void *)MPDBASE, &phypage, 0);	/* copy in common part of global mpd contents */
  sts = oz_hw486_phys_fetchlong (prctx -> ppdpa);				/* should now have same first long anyway */
  if (sts != *(uLong *)MPDBASE) oz_crash ("oz_hw_process_initctx: first mpd long %X", sts);

  /* Allocate a physical page for the reqprot page that maps the PPD and itself                  */
  ...
  oz_hw486_phys_storelong (OZ_HW_PAGEPROT_KW * 0x15400000, prctx -> rp1pa + 0xFFC); /* set last four to KW and very last to NA */

  /* Allocate two physical pages for the page table pages that map the PPD, RP1 and themselves       */
  /* Their virtual addresses will be 0xFFFFC000 and 0xFFFFB000                                       */
  /* Its first entry will map virtual address 0xFF800xxx                                             */
  /* Its last entry will map virtual address 0xFFFFFxxx                                              */
  /* They aren't part of a section, we must manually free these pages in the process_termctx routine */
  ...
  oz_hw486_phys_storelong (prctx -> pt1pa | (OZ_SECTION_PAGESTATE_VALID_W << 9) | MPD_BITS, prctx -> ppdpa + 0xFFC); /* map it in the PPD */

  phypage = oz_knl_phymem_allocpagew (OZ_PHYMEM_PAGESTATE_ALLOCSECT, 0xFFFFB);	/* allocate physical page */
  prctx -> pt2pa = phypage << 12;						/* save corresponding physical address */
  oz_hw486_phys_storelong (prctx -> pt2pa | (OZ_SECTION_PAGESTATE_VALID_W << 9) | MPD_BITS, prctx -> ppdpa + 0xFF8); /* map it in the PPD */

  /* Map the process' ppd, rp1, pt1 and pt2 pages to the process address space by filling the appropriate pt1 entries */

  oz_hw486_phys_filllong (OZ_SECTION_PAGESTATE_PAGEDOUT << 9, prctx -> pt2pa);	/* fill with pagedout entries for all pages */
  oz_hw486_phys_filllong (OZ_SECTION_PAGESTATE_PAGEDOUT << 9, prctx -> pt1pa);	/* fill with pagedout entries for all pages */

  oz_hw486_phys_storelong (prctx -> ppdpa | (OZ_SECTION_PAGESTATE_VALID_W << 9) | PT_KRW, prctx -> pt1pa + ((PROCMPDVPAGE-0)*4&0xFFF));
  ...
  oz_hw486_phys_storelong (prctx -> pt2pa | (OZ_SECTION_PAGESTATE_VALID_W << 9) | PT_KRW, prctx -> pt1pa + ((PROCMPDVPAGE-3)*4&0xFFF));
  ...
  /* Exclude the last five pages of virtual address space:                                    */
  /*   1) Exclude the hole page because we never use it for anything anyway                   */
  /*   2) Exclude the PPD page because we don't want the pager to mess with it                */
  /*      and we clean it up manually in oz_hw_process_termctx                                */
  /*   3) Exclude the rp1, pt1 and pt2 pages for same reasons as (2)                          */
  /* Our processes only have one of these ever                                                */

  sts = oz_knl_process_createpagetable (process, OZ_SECTION_EXPUP, ONEMEG-PROCBASVPAGE-5, PROCBASVPAGE);
  if (sts != OZ_SUCCESS) {
...

{
  ...
  oz_knl_phymem_freepage (prctx -> pt2pa >> 12);	/* free the four physical pages that we allocated */
  oz_knl_phymem_freepage (prctx -> pt1pa >> 12);
  oz_knl_phymem_freepage (prctx -> rp1pa >> 12);
  oz_knl_phymem_freepage (prctx -> ppdpa >> 12);
  oz_hw_smplock_clr (&oz_s_smplock_pm, pm);		/* restore smplock level */
  if (quota != NULL) oz_knl_quota_credit (quota, OZ_QUOTATYPE_PHM, 4, -1);
  oz_knl_section_increfc (prctx -> ptsec, -1);
  memset (prctx, 0, sizeof *prctx);
}
...
{
  checkpte (prctx, PROCMPDVPAGE-0, prctx -> ppdpa);	/* the mpd page should be addressible at vaddr 0xFFFFE000 */
  ...
  checkpte (prctx, PROCMPDVPAGE-3, prctx -> pt2pa);	/* the pt2 page should be addressible at vaddr 0xFFFFB000 */
}
...
{
  uLong ppdentry, ptentry, ptpagevaddr, rpentry, rppagevaddr;
  ...
  /* Read ppd entry from the ppd page just like the cpu would */

  ppdentry = oz_hw486_phys_fetchlong (prctx -> ppdpa + ((vpage >> 10) << 2));

  /* It should indicate a kernel and user writable page */

  if ((ppdentry & 7) != 7) oz_crash ("oz_hw_process_initctx checkpte: vpage %X, ppdentry %X", vpage, ppdentry);

  /* Now get the pagetable entry from the ppd entry just like the cpu would */

  ptentry = oz_hw486_phys_fetchlong ((ppdentry & 0xFFFFF000) | ((vpage & 0x3FF) << 2));
  ...
  if (((ptentry & 7) != 3) 						/* kernel writable, user no access */
   ...
   || ((ptentry & 0xFFFFF000) != paddr)) {				/* should point to correct physical page */
    oz_crash ("oz_hw_process_initctx checkpte: vpage %X, ptentry %X (ppdentry %X, ppdpa %X)", vpage, ptentry, ppdentry, prctx -> ppdpa);
  }
...
{
  OZ_Mempage vpage;
  uLong ppdentry, ptentry, virtentry;
...

{
  ...
  /* Clear out the ppd and pt pages in case we try to use them again by accident */
  /* - but not even a double-fault handler will save us from an wiped pd page    */
  /* Don't bother filling the rp1 page                                           */

  oz_hw486_phys_filllong (0x99996666, prctx -> pt2pa);	/* use even number so the 'page-present' bit will be zero */
  oz_hw486_phys_filllong (0x99996666, prctx -> pt1pa);
  oz_hw486_phys_filllong (0x99669966, prctx -> ppdpa);

  /* Free off all four pages */

  pm = oz_hw_smplock_wait (&oz_s_smplock_pm);		/* oz_knl_phymem_freepage must be called at smplock level pm */
  oz_knl_phymem_freepage (prctx -> pt2pa >> 12);	/* free the four physical pages */
  oz_knl_phymem_freepage (prctx -> pt1pa >> 12);
  oz_knl_phymem_freepage (prctx -> rp1pa >> 12);
  oz_knl_phymem_freepage (prctx -> ppdpa >> 12);
  oz_hw_smplock_clr (&oz_s_smplock_pm, pm);		/* restore smplock level */

  /* Release quota for those four physical memory pages */
...

{
  ...
  npages = ONEMEG-PROCPTRPBVPAGE-5;				/* number of pages in the section */
