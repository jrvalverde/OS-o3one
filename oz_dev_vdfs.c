...
static int wildscan_match (char const *wildcard, char const *filename, int tilde);
...
static void finishrecio (Iopex *iopex, uLong status, void (*finentry) (void *iopexv, uLong *status_r));
...
static void iodonex (Iopex *iopex, uLong status, void (*finentry) (void *finparam, uLong *status_r), void *finparam);
static uLong assign_by_name (char const *devname, OZ_Lockmode lockmode, Iopex *iopex, OZ_Iochan **iochan_r);
static uLong init_volume (int volnamelen, char const *volname, uLong clusterfactor, uLong secattrsize, const void *secattrbuff, uLong initflags, Iopex *iopex);
...
static uLong getdirid (Volume *volume, int fspeclen, char const *fspec, Fileid *dirid_r, int *fnamelen_r, char const **fname_r, char *dname_r, Iopex *iopex);
static uLong lookup_file (File *dirfile, int namelen, char const *name, Fileid *fileid_r, char *name_r, Iopex *iopex);
static uLong rename_file (File *olddirfile, char const *oldname, File *newdirfile, char const *newdirname, int newnamelen, char const *newname, int newversion, File *file, const Fileid *fileid, char *newname_r, Iopex *iopex);
static uLong enter_file (File *dirfile, char const *dirname, int namelen, char const *name, int newversion, File *file, const Fileid *fileid, char *name_r, Iopex *iopex);
static uLong remove_file (File *dirfile, char const *name, char *name_r, Iopex *iopex);
static int dirisnotempty (File *dirfile, Iopex *iopex);
static uLong create_file (Volume *volume, int namelen, char const *name, uLong filattrflags, OZ_Secattr *secattr, Fileid *dirid, File **file_r, Fileid **fileid_r, Iopex *iopex);
...
static void printe (Iopex *iopex, char const *format, ...);
...
void oz_dev_vdfs_init (int version, char const *drivername, const OZ_VDFS_Vector *vector)
...

{
  ...
  switch (funcode) {
    ...
    case OZ_IO_FS_WRITEBLOCKS: {
      ...
      if (sts != OZ_SUCCESS) oz_knl_printk ("oz_dev_vdfs io_start*: writeblocks sts %u, size %X, buff %X\n", 
	sts, iopex -> u.writeblocks.p.size, iopex -> u.writeblocks.p.buff);
...

{
  ...
  if (sts == OZ_SUCCESS) sts = rename_file (olddirfile, oldfname, newdirfile, newrname, newfnamelen, newfname, iopex -> u.rename.p.newversion, NULL, &fileid, newrname + strlen (newrname), iopex); /* rename it */
...
int ix4kbuk_validate_phypage (const OZ_Mempage *phypages, uLong phyoffs, char const *file, int line)
...
void oz_dev_vdfs_wildscan_startsubdir (Chnex *chnex, char const *name, Fileid *fileid, int wildscan_match_rc)
...
int oz_dev_vdfs_wildscan_match (Wildscan *wildscan, char const *filename)
...
/*	oldname    = old name (must include absolute version number)	*/
/*									*/
...
{
  return ((*(iopex -> devex -> vector -> rename_file)) (olddirfile, oldname, newdirfile, newdirname, newnamelen, newname, newversion, file, fileid, newname_r, iopex));
}

/************************************************************************/
/*									*/
    ...
    while (size != 0) {
      ...
      if ((blockoffs != 0) || (size < blocksize) || (size & devex -> bufalign) || (((OZ_Pointer)buff) & devex -> bufalign)) {
        if (temp == NULL) {						/* if so, allocate temp block buffer */
          temp = OZ_KNL_NPPMALLOQ (blocksize + devex -> bufalign + 1);
          if (temp == NULL) return (OZ_EXQUOTANPP);
          tempal = (uByte *)((((OZ_Pointer)temp) + devex -> bufalign) & ~((OZ_Pointer)(devex -> bufalign)));
        }
        memset (&disk_readblocks, 0, sizeof disk_readblocks);		/* read the first logical block into temp buffer */
        disk_readblocks.size = blocksize;
        disk_readblocks.buff = tempal;
        disk_readblocks.slbn = logblock;
        ...
        memcpy (buff, tempal + blockoffs, sts);				/* copy to caller's buffer */
      } else {
...

{
  ...
  /* Otherwise, write directly to disk */
    ...
    while ((size != 0) && ((blockoffs != 0) || (size & devex -> bufalign) || (((OZ_Pointer)buff) & devex -> bufalign))) {
      ...
      memcpy (tempal + blockoffs, buff, sts);				/* copy from caller's buffer */
      size -= sts;							/* reduce size left to transfer */
      ((uByte *)buff) += sts;						/* increment pointer for rest of buffer */
    ...
    if (size != 0) {							/* see if there is a last partial block */
      ...
      sts = diskio (OZ_IO_DISK_READBLOCKS, sizeof disk_readblocks, &disk_readblocks, iopex, devex -> master_iochan);
      if (sts != OZ_SUCCESS) goto writedone;
      memcpy (tempal, buff, size);					/* copy last bit from caller's buffer */
...
void oz_dev_vdfs_printk (Iopex *iopex, char const *format, ...)
...
void oz_dev_vdfs_vprintk (Iopex *iopex, char const *format, va_list ap)
