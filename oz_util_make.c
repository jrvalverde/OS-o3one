...
{
  char c, *cd, makerec[4096], *p, procbuf[65536];
  ...
  *ltarget = NULL;
  ...
  if (cd != NULL) {
    ...
    OZ_Handle h_dir, h_table;
    OZ_IO_fs_getinfo1 fs_getinfo1;
    OZ_IO_fs_getinfo2 fs_getinfo2;
    OZ_Logvalue logvalue;

    /* Open directory - we don't need to be able to even read it though */
    ...
    if (sts != OZ_SUCCESS) {
      oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u opening directory %s\n", pn, sts, cd);
      return (sts);
    }
    ...
    if (sts != OZ_SUCCESS) {
      oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u determining if %s is a directory\n", pn, sts, cd);
      oz_sys_handle_release (OZ_PROCMODE_KNL, h_dir);
      return (sts);
    }
    if (!(fs_getinfo1.filattrflags & OZ_FS_FILATTRFLAG_DIRECTORY)) {
      oz_sys_io_fs_printf (oz_util_h_error, "%s: %s is not a directory\n", pn, cd);
      oz_sys_handle_release (OZ_PROCMODE_KNL, h_dir);
    ...
    if (sts !=  OZ_SUCCESS) {
      oz_sys_io_fs_printf (oz_util_h_error, "oz_cli: error %u getting directory's device name\n", sts);
      oz_sys_handle_release (OZ_PROCMODE_KNL, h_dir);
      return (sts);
    }
    i = strlen (dirnambuff);
    ...
    oz_sys_handle_release (OZ_PROCMODE_KNL, h_dir);
    if (sts != OZ_SUCCESS) {
      oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u getting %s name\n", pn, sts, cd);
      return (sts);
    }

    /* Create OZ_DEFAULT_DIR logical name to point to it */

    sts = oz_sys_logname_lookup (0, OZ_PROCMODE_USR, oz_s_logname_defaulttables, NULL, NULL, NULL, &h_table);
    if (sts != OZ_SUCCESS) {
      oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u looking up %s\n", pn, sts, oz_s_logname_defaulttables);
      return (sts);
    }

    logvalue.attr = OZ_LOGVALATR_TERMINAL;
    logvalue.buff = dirnambuff;
    sts = oz_sys_logname_create (h_table, "OZ_DEFAULT_DIR", OZ_PROCMODE_KNL, 0, 1, &logvalue, NULL);
    oz_sys_handle_release (OZ_PROCMODE_KNL, h_table);
    if (sts != OZ_SUCCESS) {
      oz_sys_io_fs_printf (oz_util_h_error, "%s: error %u creating logical name OZ_DEFAULT_DIR\n", pn, sts);
      return (sts);
    }
  }
...
readerror:
  ...
  if (output_file != NULL) {
    ...
    if (image_name == NULL) image_name = "oz_cli.elf";
...
usage:
  ...
  oz_sys_io_fs_printf (oz_util_h_error, "%s:    [-cd <directory>]\n", pn);
  ...
  oz_sys_io_fs_printf (oz_util_h_error, "%s:   <imagename> defaults to oz_cli.elf\n", pn);
