...
##	esi = beginning of .common and .bss area to be zeroed		##
##									##
...
_start:
	...
	movl	%esi,%edi			# zero out our .common and .bss
	movl	$OZ_IMAGE_LASTADDR+1,%ecx
	subl	%edi,%ecx
	xorl	%eax,%eax
	cld
	rep
...
## Double-map oz_s_systempdata where oz_sys_pdata_array[kernelmode] is
## Any routines that try to access oz_sys_pdata_array[kernelmode] will really be accessing oz_s_systempdata
...
## This also fills in the spt with entries to map the kernel
	...
	movl	$OZ_IMAGE_LASTADDR,%eax	# point to end of loader image
	addl	$16,%eax		# round up to 16-byte boundary
	andb	$0xF0,%al
...
oz_hw_mchargx_fetch:
	...
	movl	8(%ebp),%ecx
	movw	%ds,%ax
	movw	%ax,MCHX_W_DS(%ecx)
	movw	%es,%ax
	movw	%ax,MCHX_W_ES(%ecx)
	movw	%fs,%ax
	movw	%ax,MCHX_W_FS(%ecx)
	movw	%gs,%ax
	movw	%ax,MCHX_W_GS(%ecx)
	movw	%ss,%ax
	movw	%ax,MCHX_W_SS(%ecx)
	movl	%cr0,%eax
	movl	%eax,MCHX_L_CR0(%ecx)
	movl	%cr2,%eax
	movl	%eax,MCHX_L_CR2(%ecx)
	movl	%cr3,%eax
	movl	%eax,MCHX_L_CR3(%ecx)
	movl	%cr4,%eax
	movl	%eax,MCHX_L_CR4(%ecx)
	leave
	ret
...
oz_hw_mchargx_store:
	...
	movl	%esp,%ebp
	movl	8(%ebp),%ecx
	movw	MCHX_W_DS(%ecx),%ax
	movw	%ax,%ds
	movw	MCHX_W_ES(%ecx),%ax
	movw	%ax,%es
	movw	MCHX_W_FS(%ecx),%ax
	movw	%ax,%fs
	movw	MCHX_W_GS(%ecx),%ax
	movw	%ax,%gs
	movw	MCHX_W_SS(%ecx),%ax
	movw	%ax,%ss
	movl	MCHX_L_CR0(%ecx),%eax
	movl	%eax,%cr0
	movl	MCHX_L_CR3(%ecx),%eax
	movl	%eax,%cr3
	movl	MCHX_L_CR4(%ecx),%eax
	movl	%eax,%cr4
	leave
	ret
