...

{
  ...
  /* Maybe they are zero-fill pages */
  ...
  if (section -> sectype & OZ_SECTION_TYPE_ZEROES) {
    oz_knl_printk ("oz_ldr_process_mapsection: zero filling 0x%X bytes (%uKb) at %p\n", 
	fs_readblocks.size, fs_readblocks.size >> 10, fs_readblocks.buff);
    memset (fs_readblocks.buff, 0, fs_readblocks.size);
    sts = OZ_SUCCESS;
  }

  /* Otherwise, read from the image file */
    ...
    if (sts != OZ_SUCCESS) oz_knl_printk ("oz_ldr_process_mapsection: error %u reading from kernel image\n", sts);
  }

  /* Maybe set the pages read-only */

  if (pageprot != OZ_HW_PAGEPROT_KW) {
