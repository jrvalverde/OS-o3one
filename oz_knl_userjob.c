...
/*	defdir     = default login directory				*/
...
uLong oz_knl_user_create (const char *username, OZ_Quota *quota, OZ_Seckeys *seckeys, uLong maxbasepri, char const *defdir, OZ_User **user_r)

{
  ...
  if ((user == NULL) || (i > 0)) {						/* if at end of list or existing username > new username, we need a new one */
    ...
    sts = oz_knl_logname_create (user -> lognamtbl, 
                                 ...
                                 NULL);
    if (sts != OZ_SUCCESS) {
      oz_knl_printk ("oz_knl_userjob_create: error %u creating login directory %s logical name\n", sts, defdir);
      goto rtnerr3;
    }
